...
- **작업 상황 점검**: 41차(PR #375, `65a42c7`) 머지 후 `git fetch origin main` → **`origin/main == HEAD`(클린 베이스)** 확인, 적체 draft PR 없음. 로드맵 미구현부 탐색 — Track 🏛 Standards Authorship(461-470)에서 **Phase 462**(ISO/TC 20/SC 16 표준 동향 추적)가 sandbox 완결 가능한 단일 결정적 모듈로 적합 판단해 신규 착수. Phase 401(EASA)·402(FAA)·407(ICAO) 운영/규제 정렬의 *국제 표준화(ISO)* 축 자매편.
- **Phase 462** (Track 🏛, 신규 코드) — `simulation/iso_tc20_sc16_tracker.py` + `docs/standards/ISO_TC20_SC16_TRACKING.md` — ISO/TC 20/SC 16(Unmanned aircraft systems) 표준 8종(ISO 21384-1/-2/-3/-4·21895·23665·23629-5/-7)을 SDACS 기능에 결정적으로 대응시키는 동향 추적 매트릭스. `ISOStandard` frozen dataclass(`__post_init__` standard_id↔series 온전한 접두 강제·범주/상태 allowlist·strip 정규화)·`TrackingReport`(by_category MappingProxyType 직접 생성 동결·상태합·정렬합·범주합 교차 불변식). `status` 는 프로젝트 추적 스냅샷(`AS_OF=2026-06`)·권위는 ISO 카탈로그임을 명시(시기 주장 비의존). 정렬 4/8(50%)·정직한 갭 4종(21384-1/-2 하드웨어 사양·21384-4 용어집·23665 훈련)을 `None` 으로 표면화, `test_cited_modules_exist_on_disk` 가 인용 4개 모듈(`compliance_checker.py`·`sora_category.py`·`federation_discovery.py`·`operational_intent.py`) 디스크 실재 강제·`gap ⟺ sdacs_module is None` 정직성 결속. 조회 API `find_standard`·`standards_by_category`·`standards_by_series`·`gaps`·`aligned_standards`·`published_standards`·`tracking_matrix`. 적재 시점 중복 식별자 assert 방어. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--category`·`--gaps`·`--published`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 3·MEDIUM 2·LOW 2): HIGH ①`TrackingReport.by_category` 가 직접 생성 시 MappingProxyType 미동결 → icao 자매편 규약대로 `__post_init__` 자동 래핑, ②series 검증이 bare `startswith` 라 `series='ISO 21'` 이 `standard_id='ISO 218'` 오인 → 구분자(-·공백) 또는 문자열 끝 뒤따름 강제, ③적재 시점 중복 식별자 `assert` 누락 → 카탈로그 정의 직후 추가. MEDIUM ①by_category 합 vs 상위 집계 교차검증, ②rejection 테스트 비현실 입력 → 직접 생성·합 불일치 케이스 추가. LOW ①padded/empty series 미검 분기 → 테스트 2건, ②series 구분자 거부 테스트.
- **검증**: `tests/test_iso_tc20_sc16_tracker.py` **44건 PASS**(어드바이저 반영 전 39 → 5건 추가). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 전체 수트(`-n auto`) 수집 결과 **5,341 PASS / 283 skip**, 잔여 60 fail 은 전부 선택적 의존성 미설치(`dash`·`pymavlink`·`websockets`) 사유의 환경 한계(onboard_bridge·phase10_13 WindArrow)로 본 변경과 무관 — CI 환경에서는 수집됨.
