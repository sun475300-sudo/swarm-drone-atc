...
### 추가 (feat/test) — 일일 점검 2026-06-18 (37차): ODYSSEY Track 🌏 Phase 402 FAA UTM ConOps v2.0 정렬 (USS 역할 요건 갭 분석)

- **작업 상황 점검**: 36차(PR #366, `4db7bf6`) 머지 후 `6c525d2`(public materials + 문서 갱신)까지 적층된 클린 베이스 확인. 로드맵 미구현부 탐색 결과 Track 🌏 Global Expansion(401-420)에서 직전 완료한 **Phase 401**(EASA U-space 매핑)의 자연스러운 자매 항목 **Phase 402**(FAA UTM ConOps v2 정렬 — USS 역할 요건 갭 분석)가 순수 결정적 모듈로 sandbox 구현 가능함을 확인하고 신규 착수. 401 모듈 패턴(frozen dataclass 카탈로그 + 디스크 실재 강제 + 정직 갭 표면화)을 그대로 따름.
- **Phase 402** (Track 🌏, 신규 코드) — `simulation/faa_utm_conops.py` (신규) FAA UTM Concept of Operations v2.0(2020-03)의 USS/FIMS/SDSP/OPERATOR 역할 요건 15종을 SDACS 기능에 결정적으로 대응시키는 **적합성 매트릭스**. `FaaUtmCapability` frozen dataclass 가 각 요건의 책임 역할·FAA 핵심(필수) 여부·ConOps 능력군(Strategic Deconfliction·Conformance Monitoring·Remote ID & Tracking·Constraint Management·Discovery & Synchronization·Authentication & Authorization 등)·**리포에 실재하는 제공 모듈 경로**(`sdacs_module`)를 보유. **정직성 강제 테스트**: `test_cited_modules_exist_on_disk` 가 인용된 14개 SDACS 모듈 경로(`operational_intent.py`·`path_deconflict.py`·`compliance_checker.py`·`remote_id.py`·`telemetry_recorder.py`·`notam_manager.py`·`faa_laanc.py`·`federation_discovery.py`·`autonomous_negotiation.py`·`emergency_protocol.py`·`drone_registry.py`·`weather.py`·`terrain_awareness_system.py`)의 디스크 실재를 강제(허위 인용 차단). **정직 갭**: FAA 정부 게이트웨이(FIMS) 데이터 교환은 `required=True`·`sdacs_module=None` 으로, 시뮬레이터 범위 밖 미충족 핵심 요건임을 명시 표면화(`test_fims_data_exchange_is_a_known_gap`). 전체 충족 14/15(93%)·핵심 8/9(89%, FIMS 갭으로 미완)·역할별(USS 11/11·FIMS 0/1·SDSP 2/2·OPERATOR 1/1)을 `conformance_report()` 가 결정적 집계. 조회 API `capabilities_by_role`·`required_capabilities`·`gaps`·`implemented_capabilities`·`capability_matrix`(도구 간 교환용 JSON 행)·`find_capability`. `ConformanceReport`(frozen, `__post_init__` 카운트 불변식 검증·`by_role` MappingProxyType 읽기 전용). 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--conformance`·`--role`·`--gaps`·`--required`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 2·MEDIUM 3·LOW 2): HIGH ①`ConformanceReport` 를 평범한 dict `by_role` 로 **직접 생성**하면 frozen 임에도 `r.by_role["X"]=…` 무음 변조 가능(공개 factory `conformance_report()` 만 MappingProxyType 래핑, 기존 read-only 테스트가 factory 경로만 검증해 우회 미탐지) → `__post_init__` 에서 `MappingProxyType` 아니면 강제 래핑 + 직접 생성 읽기 전용 회귀 테스트 추가. ②`capability_matrix()` 가 가변 dict 행 반환 → "도구 간 교환용 JSON 행" 목적상 `json.dumps` 직렬화 필요로 plain dict 의도 유지(자매 401 `service_matrix` 와 동일 판단)임을 명시. MEDIUM(by_role 교차 일관성 미검증·`_ROLE_NAMES` 미사용·직접 생성 가변성 테스트 갭)·LOW(`find_capability` O(n)·내부 공백 id)는 현 15종 규모에서 영향 없거나 ①로 해소 — 자매 모듈 일관성·YAGNI 고려해 추가 변경 보류.
- **검증**: `tests/test_faa_utm_conops.py` **39건 PASS**(요건 검증·카탈로그 무결성·디스크 실재·FIMS 갭 명시·역할별 분할·적합성 불변식·매트릭스 정렬/결정성 + 어드바이저 반영 직접 생성 read-only). 인접 회귀 `test_uspace_service_map`(37)·`test_geo_zones`(39) **76건 PASS** 재확인. 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. CLI `--matrix`·`--conformance` 실측 출력 확인. 본 컨테이너 최소 의존성(pytest·numpy·pyyaml·xdist·cov) 설치 → 전체 수트는 CI 수집.
