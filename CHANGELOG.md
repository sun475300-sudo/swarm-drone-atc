...
### 추가 (feat/test) — 일일 점검 2026-06-19 (37차): ODYSSEY Track 🏛 Phase 470 표준화 기고 추적 대시보드

- **작업 상황 점검**: 36차(`6c525d2`) 머지 후 `git fetch origin main` → **브랜치 == origin/main (클린 베이스)** 확인. 적체 드래프트 없음. 본 컨테이너는 신규 클론이라 의존성 미설치 → 전체 수트 수집 시 60건 실패가 전부 **환경 의존성 누락**(pandas·websockets·asyncpg·pytest-asyncio)임을 진단, 해당 모듈 설치 후 timescale·monte_carlo·ws_bridge 36+건 재통과 확인 → 베이스라인 건전(회귀 아님). Track 🏛 Standards & Policy(461-480)에서 다음 sandbox 가능 항목인 **Phase 470**(표준화 기고 추적 대시보드)을 신규 착수.
- **Phase 470** (Track 🏛, 신규 코드) — `simulation/standardization_tracker.py` (신규) Standards & Policy 트랙이 산출하는 **표준화 기고**(ASTM F38·ISO/TC 20/SC 16·GUTMA·EASA·국토부 K-드론·국가 안전기준·ASTM F3411 Remote ID 등)의 진행 상태를 단일 SSoT 로 추적·대시보드화. **중복 없는 추적 계층**: 기고의 *내용* 은 산출물 파일(`docs/standards/*`·`docs/schemas/*`)이 유일 출처이며 본 모듈은 내용을 복제하지 않고 진행 메타데이터(대상 표준 기구·발신 Phase·상태·산출물 상대 경로)만 보유해 파일을 가리킨다(`Contribution` frozen dataclass 10건, 결정적 순서). 단조 상태 모델 PLANNED→DRAFTING→SUBMITTED→PUBLISHED→ADOPTED(순서형 가중치)로 `progress()` 가 전체 진행률을 가중 평균(현재 9/40 = **22.5%**). **검증 가능한 정직성**(핵심 가치): `validate_registry` 가 ①PUBLISHED 이상 기고는 산출물 경로 명시 + 디스크 실재(Phase 465 `SDACS_BENCHMARK_SUITE.md`·466 `telemetry.schema.json`·467 `INCIDENT_INVESTIGATION_REPORT.md` 3건 실재 검증), ②경로 명시한 모든 기고의 파일 실재, ③식별자 유일·상태 enum 적합·body/title/phase/id 비공백·phase `TRACK-번호` 형식을 결정적 검사 — 옮겨지거나 사라진 산출물·허위 PUBLISHED 를 위양성 없이 잡아낸다. `ValidationResult(is_valid·errors·warnings)` 계약은 sibling `scenario_schema.py`(GENESIS 322)·`standard_scenarios.py`(465)와 동일. 집계 `status_rollup`(상태별·합=총건수)·`body_rollup`(기구별)·JSON `manifest`(JSON 직렬화·결정적)·`markdown_table` 대시보드. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--list`·`--validate`·`--manifest`·`--markdown`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 2·MEDIUM 1·LOW 1): HIGH ①`ValidationResult.summary()` 가 건수와 무관히 단수형(`INVALID (2 error)`) 출력 → 복수형 분기(`2 errors`), ②기본키 `contribution_id` 가 비공백 검사에서 누락돼 빈/공백 ID 가 무검증 통과(나머지 body/title/phase 는 검사) → id 비공백 검사 추가. MEDIUM `markdown_table` 셀 값에 파이프(`|`) 포함 시 표가 조용히 깨짐 → `_escape_md` 로 이스케이프. LOW shipped 레지스트리 진행률 회귀 핀(`progress()==0.225`)으로 상태 무음 변동 감지. (어드바이저 MEDIUM phase 정규식 compound-name 취약성·LOW CLI fallthrough 는 현 데이터에서 무해·YAGNI 로 미반영, 검토 결과 기록.)
- **검증**: `tests/test_standardization_tracker.py` **31건 PASS**(어드바이저 반영 전 27 → 4건 추가: 빈 id 거부·summary 복수형·markdown 파이프 이스케이프·진행률 핀). 전체 수트 수집 **5,442건**, 신규 모듈 격리 통과·회귀 무영향. 대상 기존 `.py` 무수정 순수 추가. CLI `--validate` = VALID·`--manifest` progress 0.225/count 10/valid true 실측. 본 컨테이너 최소 의존성 설치 후 신규 phase 격리 검증, 전체 수트는 CI 수집.
