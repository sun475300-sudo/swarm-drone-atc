...
### 추가 (feat/test) — 일일 점검 2026-06-19 (40차): ODYSSEY Track 🌏 Phase 409 다국 BVLOS 규제 비교 (한·미·EU·일)

- **작업 상황 점검**: `git fetch origin main` → **HEAD == origin/main (`6c525d2`) 클린 베이스** 확인(로컬 브랜치도 완전 동기화). 열린 PR triage 결과 **적체 드래프트 7건**(#372 Phase 407·#371 Phase 403·#370 Phase 470·#369·#368 둘 다 Phase 402 **중복**·#280 Phase 207·#283 perf) + dependabot 다수. 진행 중 phase(402·403·407·470)와 **충돌하지 않는** 미구현 항목으로 Track 🌏 Global Expansion(401-420)의 **Phase 409**(다국 규제 비교 대시보드)를 신규 착수.
- **Phase 409** (Track 🌏, 신규 코드) — `simulation/bvlos_regulation_compare.py` (신규) 드론 BVLOS 운용을 허가하는 **한국·미국·EU·일본 4개 관할**의 규제 요건을 **동일한 6개 비교 축**(법적 허가 경로·운영 위험 평가·Remote ID·조종 자격·기체 인증·인구밀집지/제3자 상공 BVLOS)으로 정렬해 대조하는 결정적 데이터 모듈. `JurisdictionRegulation`·`BvlosRequirement` frozen dataclass — 각 요건이 권위 있는 공개 규제 문서를 `citation` 으로 인용(KR 항공안전법 §129·US 14 CFR Part 107/108 NPRM·EU Reg.(EU) 2019/947 SORA·JP 改正航空法 Level 4)하고 관할별 `as_of` 스냅샷 시점 보유. `__post_init__` 가 전 비교 축 정확히 1회 보유(빈칸·중복 거부)·관할 코드 allowlist·`as_of` 월 01-12·인용 비공백 불변식 강제. 조회 API `find_jurisdiction`·`compare_dimension`·`comparison_matrix`(도구 간 교환용 TypedDict JSON 행). **정직 공시**: 법률 자문 아닌 *기능적 요약*, `sdacs_support` 는 관할 요건 자가 평가에 실재로 쓰이는 리포 모듈(KR `special_flight_approval.py`·US `flight_plan_filing.py`·EU `airspace_class.py`)을 가리키고 **일본은 `None` 으로 갭 표면화**(`test_cited_sdacs_modules_exist_on_disk` 가 인용 경로 디스크 실재 강제) → 지원 3/4(75%). Phase 402(FAA)·403(EASA)·407(ICAO) 자가 평가와 비경쟁(관할 *간* 대조라는 별도 축). 공개 비교 문서 `docs/standards/BVLOS_REGULATION_COMPARISON.md` 동반. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--jurisdiction`·`--dimension`·`--support`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 2·MEDIUM 3·LOW 3): HIGH ①`as_of` 검증이 월 범위를 안 봐 `2026-13`·`2026-00` 무음 수용 → `1 ≤ month ≤ 12` 강제 + 파라미터화 거부 테스트 3건, ②`compare_dimension`·`comparison_matrix` 가 가변 dict 반환으로 하류 공유 상태 변조 가능(CLAUDE.md 불변성 위반·uspace 선례 이탈) → `MappingProxyType` 읽기 전용 래핑 + 읽기 전용 단언 테스트 2건. MEDIUM ①`SupportReport` 의 `supported>total` 죽은(도달 불가) 검사 제거, ②`JURISDICTION_CODES`↔`REGULATIONS` 무음 발산(allowlist 신규 코드 추가 시 무음 누락) → 동기 테스트 추가, ③weak 반환 타입 `tuple[dict[str,object],...]` 가 `type:ignore` 2개 유발 → `MatrixRow`/`MatrixCell` TypedDict 로 교체해 ignore 제거. LOW ①`JURISDICTION_CODES` 정렬 import-time 단언, ②월 범위 테스트(기 존재 확인), ③CLI 포맷 사소 불일치(의도된 레이아웃 차이로 유지).
- **검증**: `tests/test_bvlos_regulation_compare.py` **40건 PASS**(어드바이저 반영 전 34 → 6건 추가: 코드 동기·읽기 전용 매핑 2·월 범위 3). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 최소 의존성(pytest·numpy·pyyaml) 설치 → 전체 수트는 CI 수집.
