...
### 추가 (feat/test) — 일일 점검 2026-06-19 (46차): ODYSSEY Phase 490 — 디지털 유산 10년 재현성 체크리스트

- **작업 상황 점검**: 신규 컨테이너에서 의존성 신규 설치(`requirements.txt`, PyJWT RECORD 충돌은 `--ignore-installed PyJWT` 로 우회 + `pytest-xdist` 보강) 후 baseline 전체 회귀 **5,669 pass / 280 skip / 0 fail**(97.66s) 독립 재현 GREEN. 열린 이슈 0건. 로드맵 Continuum 트랙(481-500)에서 *코드 작업거리가 있는* 미구현 칸을 식별 → **Phase 490** 을 Phase 481/488/489 자매 패턴(결정적 정책·부수효과 0·자문)으로 구현.
- **Phase 490** (Track ♾️ Continuum) — `simulation/legacy_readiness.py` + `docs/standards/DIGITAL_LEGACY_CHECKLIST.md`. "원저자가 손을 뗀 뒤 10년에도 **재현·인용·법적 사용이 가능한가**"를 **결정적 체크리스트**로 명문화(같은 리포 상태=같은 판정). 8개 기준 × 7개 차원(reproducibility·licensing·archival·identity·documentation·security·governance) × 심각도 가중(CRITICAL 3·IMPORTANT 2·RECOMMENDED 1). `LegacyCriterion` frozen dataclass(증거 = 리포 상대 경로 튜플)·`check_criterion`(모든 증거 파일 디스크 실재 + 아카이브 기준은 **Phase 489 내구 게이트 위임**, 테스트 주입 가능한 `archive_gate`)·`assess_legacy_readiness`(CRITICAL 미충족 시 `NOT_READY`·전부 충족 `READY`·그 사이 `PARTIAL`, 가중 충족 비율 점수). **증거는 디스크 실재만 인정**(준비 예정은 불인정·정직성), **CRITICAL 한 칸이라도 비면 READY 아님**, **아카이브 차원은 중복 로직 없이 Phase 489 재사용**. **정직 공시**: `--status` 판정 `NOT_READY (58.8%)` — `pyproject.toml`·`package.json`·`CITATION.cff` 가 MIT 를 *선언*하나 최상위 `LICENSE` 전문 부재 + DOI 미발급(Phase 489 `AT_RISK`)로 CRITICAL 2칸(`license-file`·`archival-durable`) 미충족. 회귀 핀(`test_live_repo_not_ready_until_license_and_archive`)이 LICENSE 추가·DOI 발급 시 *의도적으로* 깨져 체크리스트 갱신 강제. **자문이지 집행 아님**(누락 자산 생성 안 함·부수효과 0)·무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--checklist`·`--status`·`--manifest`). code-reviewer 어드바이저 HIGH 2(① `test_partial_when_only_recommended_missing` 가 이름과 달리 `score>0` 만 검증·`PARTIAL`/`READY` 경로 전무 → 아카이브 게이트 주입점 신설로 PARTIAL/READY/게이트 실패 3경로 실검증, ② live 핀 두 개의 *반대 방향* 혼동 → 방향 주석 구분)·MEDIUM 2(게이트가 `repo_root` 무관함 docstring 명시·verdict 경로 커버리지)·LOW 2(`all_satisfied`→`no_unmet` 개명·공유 증거 주석) 반영. 단위 **30건 PASS**(모듈 커버리지 99%).
- **점검 발견(사용자 보고)**: (1) 열린 PR **16건** — Dependabot 13(starlette/pydantic-core/matplotlib/electron/playwright 등) + 적체 #283(perf) + draft #280(Phase 207) — 직전 점검들의 "열린 PR 0건" 기록과 불일치, 의존성 갱신 적체는 Phase 481 게이트 기준 검토 권장. (2) 본 작업 브랜치(`claude/fervent-babbage-yr8zx6`)와 `origin/main`(`19ef86d`, 2026-06-05)이 **공통 조상 0인 독립 히스토리** — 머지 토폴로지 확인 필요.
- **검증**: 신규 30건 포함 통합 후 전체 회귀 **5,695 pass / 280 skip / 0 fail**(90.14s). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향.
