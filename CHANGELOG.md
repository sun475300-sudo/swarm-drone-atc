...
### 추가 (feat/test) — 일일 점검 2026-06-24: ODYSSEY Track 🔬 Phase 455 — ML 데이터 관리(Data Management) 적합성 게이트

- **작업 상황 점검 + 적체 일원화**: 신규 세션 컨테이너에서 작업 브랜치(`tp1sky`, HEAD `e7f4bc6`)가 main 대비 336 커밋 선행·origin 동기 상태 확인. 병행 세션이 본 HEAD 기준으로 만든 미머지 적체 draft **PR #439**(`claude/fervent-babbage-8l35k2`, Phase 404·405·411·452·453·454·473 7건 일원화, 본 HEAD 의 클린 상위집합)을 fast-forward 흡수해 적체 7 Phase 를 단일 작업 브랜치로 일원화(379건 PASS 재검증). 그 위에 451-460 트랙에서 *유일하게 남은 서로소 코드화 칸* **Phase 455** 를 자매 패턴으로 신규 구현.
- **Phase 455** (Track 🔬 Formal & Research Frontier, 신규 코드) — `simulation/ml_data_management.py` + `docs/standards/ML_DATA_MANAGEMENT.md`. EASA Concept Paper(Level 1&2 ML, Issue 02 2024)의 러닝 어슈어런스 **W-shape 좌측 팔(데이터 라이프사이클 관리)** 을 SDACS 의 ML(RL) 자산이 어디까지 충족하는가를 **결정적**으로 평가하는 자문 게이트. Phase 451 이 핵심 갭으로 지목한 `learning_process_verification` 의 *입력측*(데이터)을 세분 — 452(`rl_generalization_protocol`)가 *검증 방법*(W 우측)을 게이트한다면 455 는 *데이터 관리*(W 좌측)를 게이트. `DataObjective` frozen dataclass 12종을 4단계(`requirements`·`collection`·`preprocessing`·`verification`)로 분류, `__post_init__` snake_case id·비공백·stage/criticality/status enum 강제 + **정직성 결속** `status==gap` ⟺ `evidence is None`(그 외엔 실재 경로 인용 강제). `DataManagementReport`(frozen, `by_stage` `MappingProxyType` 읽기 전용·카운트 비음수·단계 합 교차검증·**미지 단계 키 거부**). `audit_cited_artifacts` 라이브 감사가 인용 산출물 디스크 실재를 강제하고, 부재 시 `assess_data_management` 가 해당 목표를 `gap` 으로 **자동 강등**(근거도 None 으로 제거해 정직성 결속 유지) → 정적 카탈로그와 실측 불일치를 정직 표면화. 판정 우선순위(서로소): 기반 갭→`DATA_NOT_READY`·기반 부분→`DATA_AT_RISK`·기반 전부 충족→`DATA_MANAGED`(기반 0 이면 공허 MANAGED 차단 NOT_READY). **정직 공시**(CLAUDE.md): SDACS RL 학습 데이터는 *시뮬 생성*(시나리오 코퍼스 10종 + 도메인 무작위화 분포)이고 실 비행 로그 미수집(Track A HW 의존) → **실세계 데이터 출처가 기반 갭** → 현 리포 판정 **`DATA_NOT_READY (71%)`**(충족 7·부분 3·갭 2/총 12, 기반 3/5). 데이터 관리 SW 골격(요구 명세·홀드아웃 분리·결정적 파이프라인)은 갖췄으나 실측 대표성 입증 전까지 인증 미완임을 정직 보고(준비도 ≠ 완수, Phase 451 보수적 33%·Track A 잔여와 정합). 무작위성 0·부수효과 0·자문이지 집행 아님·기존 모듈 무수정 순수 추가. CLI(`--objectives`·`--report`·`--audit`·`--gaps`·`--foundational`·`--stage`). **code-reviewer 어드바이저 반영**. 단위 **77건 PASS**.
- **검증**: `tests/test_ml_data_management.py` **77건 PASS**(카탈로그 무결성·정직성 결속·디스크 실재 강제·DataObjective 검증·산출물 부재 강등·verdict 7우선순위·DataManagementReport 카운트/단계키/읽기전용·data_matrix 자기서술·결정성·보수적 점수<100%·CLI 7종, 0.26s). `python -O` 최적화 임포트 정상(무결성 게이트 명시적 raise). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 흡수한 적체 7 Phase 모듈(452·453·454 등) 동반 회귀 379건 PASS.
- **점검 발견(사용자 결정 필요)**: ① 본 작업 브랜치가 PR #439(404·405·411·452·453·454·473) + Phase 455 를 단일 브랜치로 일원화 → 본 PR 머지 후 적체 데일리 draft **PR #429–439**(11건, 상호 중복) close 권고. ② **Dependabot 14건 적체**(#267-279·#367·#426·#427) — Phase 484 가 현 electron 핀 EOL 공시(#426 electron 39→42 우선). ③ **GitHub 보고 취약점 4건**(2 high·2 low) 미해소 — 전부 사용자 승인 필요.
...
- **Phase 454** (Track 🔬, 신규 코드) — `simulation/ml_application_classification.py` + `docs/standards/ML_APPLICATION_CLASSIFICATION.md`. SDACS 의 *학습 기반(ML)* 자산 4종(PPO 충돌 회피 `src/rl/ppo_collision.py`·하이브리드 APF+RL `src/autonomy/hybrid_collision_avoidance.py`·도메인 무작위화 `src/training/domain_rand.py`·시뮬-실측 보정 `src/training/sim_real_gap.py`)이 EASA Concept Paper(Level 1&2 ML, Issue 02 2024)·AI Roadmap 2.0 의 **AI Level 분류** 축에서 어디에 놓이는가를 결정적으로 판정. 인증 경로의 *부담* 은 Level 에 따라 급변(Level 1 경량 ↔ Level 3 러닝 어슈어런스 전체)하므로 "각 ML 자산이 어느 Level 인가" 가 인증 가능성 논의의 첫 관문. **`POLICY_MATRIX`**(과업 배분 assistance/cooperation/automation × 권한 보유 retained/reduced/none = **9칸 전함수** → Level 1A..3B)를 `classify_level` 이 결정적 적용 — 가족(1/2/3)은 과업 배분이, 하위 문자(A/B)는 권한 보유가 가른다(테스트가 9칸 전수 일치 강제). `MLConstituent` frozen dataclass(snake_case id·텍스트 비공백·enum 강제·**정직성 결속** `human_authority != none` ⟺ 비-AI `override_authority` 실재 경로 인용, `none` ⟺ 인용 금지 → 권한자 없는 감독 주장 차단), `ClassificationReport`(frozen, `by_level`/`by_family` MappingProxyType 읽기 전용·미지 Level 키 거부·가족 합 교차검증). `test_cited_modules_exist_on_disk`·`test_cited_override_authorities_exist_on_disk` 가 인용 ML 4종 + 권한자 4종 디스크 실재 강제(허위 분류 차단). **핵심 발견(정직 공시)**: 전 ML 자산이 **Level 1A**(자문, 최저 인증 부담) — ML 은 항상 자문이고 안전-결정권은 결정적 APF+CBS 5계층 안전망이 보유하므로. 낮은 Level 은 결함이 아니라 "ML 을 안전-크리티컬 결정에 신뢰하지 않음" 설계 선택의 보상(Level 2/3 러닝 어슈어런스 전체 부담 회피). 무작위성 0·부수효과 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--levels`·`--policy`·`--constituent`). **code-reviewer 어드바이저 반영**: MEDIUM(CLI `--constituent` 미지 id `KeyError` 무처리 → `stderr` 정리·exit 1)·LOW(`find` 식별자 단언 강화·미지 id CLI 회귀 테스트 추가). HIGH `__slots__` 제안은 검토 결과 *선언 슬롯 필드는 `object.__setattr__` 로 여전히 변형 가능* 해 우회를 막지 못함을 실측 확인 → 오도 주석 회피 위해 미채택, 대신 디스크 실재 검사 테스트 위임·`POLICY_MATRIX` 익명 dict 불변 근거를 주석으로 명문화. 단위 **54건 PASS**.
- **검증**: `tests/test_ml_application_classification.py` **54건 PASS**(dataclass 불변식·정직성 결속·9칸 정책 전수 일치·디스크 실재 강제·리포트 카운트/가족/읽기전용 불변식·매트릭스 자기서술/읽기전용/정렬·결정성·전 자산 1A·CLI 6종, 0.10s). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 최소 의존성(pytest·numpy·pyyaml) 설치 → 전체 수트는 CI 수집.
- **점검 발견(사용자 결정 필요)**: ① **열린 PR 23건 적체** — Dependabot 13건(#267-279)+#367 + perf #283(핫루프 힙 할당) + 일일점검 ODYSSEY 일원화 draft 다수(#424·#425·#428·#429-435, 상호 중복 일원화). #435 가 Phase 452·453·404·405·411·473 을 일원화 중이나 **2일+ 미머지**. ② **GitHub 보고 취약점 4건(2 high·2 low)** 미해소. 머지·triage·중복 close·취약점 패치는 모두 **사용자 승인 필요** — 적체의 근본 원인은 추가 코드 부재가 아니라 머지 결정(issue #409 와 동일 진단, 적체 규모는 15→23 으로 악화).
