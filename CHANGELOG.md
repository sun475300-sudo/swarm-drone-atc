...
### 추가 (feat/test) — 일일 점검 2026-06-19 (38차): ODYSSEY Track 🌏 Phase 403 EASA EU 2019/947 운영 카테고리 판정

- **작업 상황 점검**: 36차(PR #366, `4db7bf6`) 머지 후 신규 컨테이너에서 `git fetch origin main` → **`origin/main == HEAD`(`6c525d2`, 클린 베이스)** 확인. 의존성 신규 설치(`requirements.txt` + pytest-xdist·hypothesis·jsonschema, blinker RECORD 충돌은 `--ignore-installed` 우회), 전체 수트 **5,501건 수집** 베이스라인. **적체 드래프트 발견**: 병행 세션이 남긴 미머지 draft PR — **#370**(2026-06-19 Phase 470 표준화 기고 추적 대시보드, CI 미실행)·**#368·#369**(2026-06-18 Phase 402 FAA UTM ConOps v2.0 — 동일 phase 중복본 2건)·**#367**(dependabot starlette). 본 세션은 이들과 **비중첩** 인 Track 🌏 Global Expansion(401-420)의 **Phase 403**(EASA 운영 카테고리 판정)을 신규 착수하고, 적체 드래프트는 머지/triage 판단이 필요해 보고만 한다(드래프트·중복·CI 미실행이라 자동 머지 보류).
- **Phase 403** (Track 🌏, 신규 코드) — `simulation/sora_category.py` (신규) EASA **EU 2019/947** 운영 카테고리(Open/Specific/Certified) 결정적 판정. GENESIS 302 `soraAssess`(시뮬레이터 JS, JARUS SORA 2.0 표 → SAIL 만 산정) 위에 **세 운영 카테고리 + Open 하위분류(A1·A2·A3)** 를 얹는 Python 계층으로, K-드론·FAA UTM 정렬(Phase 401·402·406·408)을 잇는 EASA 규제 분류 축. 판정 우선순위: ①**Certified 강제 트리거**(인원 수송·고위험 위험물·군중 상공+특성 치수 ≥3 m, EU Art. 4) → ②**Open 적격**(MTOM<25kg·VLOS·고도≤120m·군중 상공/위험물 없음 + 유효 A1/A2/A3, UAS.OPEN.020/030/040) → ③**Specific**(SORA 산정, 최종 GRC>7 시 Specific 범위 초과로 Certified 격상). `SORA_IGRC`·`SORA_SAIL_TABLE` 은 `swarm_3d_simulator.html` JS 와 **동일 복제**(동일 입력=동일 SORA 출력, CLAUDE.md "수치 불일치 금지" 준수). 완화 M1 지오펜스·M3 ERP(각 −1, GRC≥1 클램프), ARC populated/assembly→c, TMPR c→b. C-class 는 EU 2019/945 질량 게이트로 C0-C3 파생(`c_class` 명시 우선). `OperationProfile`·`SoraResult`·`CategoryAssessment` frozen dataclass(`__post_init__` 입력 검증)·`derive_c_class`·`assess_sora`·`assess_category`·`summary`. 무작위성 0·기존 모듈 무수정 순수 추가. 공개 문서 `docs/certification/EU_OPERATIONAL_CATEGORY.md` 동반.
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 2·MEDIUM 3·LOW 3): HIGH ①JS `soraAssess` 는 군집을 BVLOS 가정(`bvlos !== false`)인데 Python 기본이 `vlos=True` 라 *기본 인자 호출이 JS 와 다른 iGRC*(sparse: JS 3 vs Py 2)를 내어 파리티 위반·위험 과소평가 → **`vlos` 기본값을 False(BVLOS)로 전환**(Open 의도 테스트 8건에 `vlos=True` 명시 추가), ②`SORA_SAIL_TABLE` 의 ARC-a/d 열이 TMPR 로 항상 'b' 귀결되어 미사용 → JS 표 동일성·향후 확장 위해 보존하되 사유 주석 명시. MEDIUM ①군중 상공(over_assembly)인데 밀도가 낮으면 ARC 가 과소(ARC-b) 산정 → over_assembly 면 ARC-c 로 올리고 **TMPR c→b 경감을 적용하지 않음**(소형<3m Specific 분기가 SAIL II→IV 보수 상향), ②`dangerous_goods` 가 `_certified_trigger`·`_open_hard_constraints` 양쪽 검사라 후자가 도달 불가 dead code → 의도적 방어 중복 주석, ③C5/C6/C7 범위 밖을 `ValueError` 메시지·docstring 에 명시. LOW ①`arc_letter`(표 키) vs `SoraResult.arc`(표시용 'ARC-' 접두) 혼용 금지 주석, ②over_assembly+sparse ARC-c 검증 테스트 추가, ③`reasons` 필드 `field(default_factory=tuple)`→`()`.
- **검증**: `tests/test_sora_category.py` **51건 PASS**(어드바이저 반영 전 49 → BVLOS 파리티·over_assembly ARC-c 2건 추가). C-class 질량 게이트·Open A1/A2/A3·Specific 강등·Certified 3종 트리거·SORA 표 정합·입력 검증·결정성·summary 커버. 사촌 회귀 `test_airspace_class`·`test_geo_zones`·`test_uspace_service_map`·`test_special_flight_approval` **126건 PASS**, 전체 수트 **5,552건 수집**(+51). CI ruff(`E9,F63,F7,F82,W,I`) 클린. 대상 기존 `.py`·JS 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 의존성 설치 후 sora 모듈 단위 GREEN, 전체 수트는 CI 수집.
