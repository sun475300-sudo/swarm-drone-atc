...
### 추가 (feat/test) — 일일 점검 2026-06-18 (37차): ODYSSEY Track 🌏 Phase 402 FAA UTM ConOps v2.0 USS 역할 갭 분석

- **작업 상황 점검**: 36차(PR #366, `4db7bf6`) 머지 후 브랜치(`claude/fervent-babbage-st5s0i`)가 origin/main 과 동기·클린 베이스임을 확인(테스트 5,415개 정상 수집). 로드맵 활성 프런티어인 Track 🌏 Global Expansion(401-420)에서 36차가 착수한 Phase 401(EASA U-space)의 *미국* 대응이자 다음 순번인 **Phase 402**(FAA UTM ConOps v2.0 USS 역할 갭 분석)를 sandbox 가능 신규 항목으로 착수.
- **Phase 402** (Track 🌏, 신규 코드) — `simulation/faa_uss_roles.py` (신규) FAA UTM ConOps v2.0 의 USS(UAS Service Supplier) 역할 17종을 SDACS 모듈에 결정적으로 대응시키는 **적합성 매트릭스**. `USSRequirement` frozen dataclass 카탈로그가 각 역할의 기능 범주(Operation Management·Airspace & Constraints·Identification & Surveillance·Information Services·Inter-USS / Network·Safety & Contingency 6종)·`core`(USS Network 참여 기반 역할 여부)·**리포에 실재하는 제공 모듈 경로**(`sdacs_module`)를 보유 — 대응 모듈이 없는 운영자 자격 검증(`operator_credentialing`)·공공안전/법집행 데이터 접근(`public_safety_access`)은 `None` 으로 **갭** 을 정직 표면화. **정직성 강제 테스트**: `test_cited_modules_exist_on_disk` 가 인용 모듈 경로(`operational_intent.py`·`path_deconflict.py`·`compliance_checker.py`·`notam_manager.py`·`faa_laanc.py`·`geofence_manager.py`·`remote_id.py`·`telemetry_recorder.py`·`weather.py`·`airspace_capacity.py`·`federation_discovery.py`·`federation_audit.py`·`traffic_coordinator.py`·`emergency_protocol.py`)의 디스크 실재를 강제. 핵심 USS Network 역할 7종(operation intent 관리·strategic deconfliction·conformance monitoring·network Remote ID·constraint 전파·inter-USS 통신·DSS 동기화)은 100% 충족(7/7)·전체 15/17(88%)을 `conformance_report()` 가 결정적 집계. core/보강 구분이 ConOps 강조점을 따른 *프로젝트 해석*(FAA 명시 목록 아님)임을 docstring 에 명시. 조회 API `requirements_by_category`·`core_requirements`·`gaps`·`implemented_requirements`·`role_matrix`(도구 간 교환용 JSON 행)·`find_requirement`. `ConformanceReport`(frozen, `__post_init__` 카운트 불변식). 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--conformance`·`--category`·`--gaps`·`--core`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 2·MEDIUM 3·LOW 1): HIGH ①`ConformanceReport` 가 `core_implemented≤implemented` 불변식을 누락해 `ConformanceReport(10,3,7,5,4,{})`(충족 핵심 4 > 충족 전체 3)같은 모순 상태를 수용 → 핵심은 전체의 부분집합이므로 불변식 추가, ②`role_matrix()` 행이 가변 plain dict 라 `by_category` 의 `MappingProxyType` 보장과 불일치 → 각 행을 `MappingProxyType` 읽기 전용으로 래핑(타입을 `Mapping` 으로). MEDIUM ①`inter_uss_communication`·`operation_planning` 의 의도된 `operational_intent.py` 공유에 명시 주석, ②`data_archiving` 의 Inter-USS / Network 범주 분류 근거 주석, ③`core` 정의가 FAA 명시 목록이 아닌 프로젝트 해석임을 docstring 에 명시. LOW: CLI 5개 분기 스모크 테스트 추가(parity 개선).
- **검증**: `tests/test_faa_uss_roles.py` **46건 PASS**(어드바이저 반영 전 38 → 8건 추가: core>implemented 거부·role_matrix 행 읽기 전용·CLI 5분기+기본+category 스모크). 인접 모듈 `test_operational_intent`(?)·`test_federation_discovery` **38건 PASS** 재확인(영향 없음). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 의존성(pytest·numpy·pyyaml·simpy·plotly·hypothesis 등) 설치 후 5,415개 정상 수집, 전체 수트는 CI 측정.
