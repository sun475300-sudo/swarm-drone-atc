...
### 추가 (feat/test) — 일일 점검 2026-06-24 (53차): ODYSSEY Track 🌏 Phase 405 — 국제 벤치마크 제출 (SDACS · BlueSky · U-TRAFMAN 비교)

- **작업 상황 점검**: 신규 세션 컨테이너에서 최소 의존성(pytest·numpy·pyyaml) 설치. `git log` 상 직전 일일 점검(PR #423, 머지 커밋 `d9f1067`)은 ODYSSEY 적체 draft 6건(Phase 410·451·461-472·491·492·500)을 일원화한 상태. ODYSSEY Track 🌏 Global Expansion(401-420)에서 401·402·403·406-410 은 완료됐으나 **Phase 404·405 가 미착수 공백**으로 남아 있었음(낮은 번호 열린 칸). 404(README EN 완역)는 143KB 문서 번역으로 단일 결정적 산출물에 부적합하므로, 자매 패턴(Phase 409 BVLOS 비교)으로 코드화 가능한 **Phase 405**(국제 벤치마크 비교)를 신규 구현. 의존 선행 Phase 465(`SDACS-SBS-10` 공개 표준 스위트)는 이미 완료돼 제출 단위로 결속 가능.
- **Phase 405** (Track 🌏 Global Expansion, 신규 코드) — `simulation/benchmark_comparison.py` + `docs/standards/INTERNATIONAL_BENCHMARK_COMPARISON.md`. SDACS 를 공개 오픈소스 시뮬레이터 **BlueSky**(TU Delft 유인 ATM/ATC, Python 3, GNU GPL v3 — Hoekstra & Ellerbroek, *BlueSky ATC Simulator Project*, ICRAT 2016)·**U-TRAFMAN**(Univ. Castilla-La Mancha I3A-NavSys 무인 UTM, ROS+Gazebo+Matlab, GNU GPL v3 — Jover, Casado & Bermúdez, *U-TRAFMAN*, SoftwareX 2025) 과 **동일 비교 축 8종**(`domain`·`license`·`conflict_method`·`traffic_model`·`swarm_support`·`weather_model`·`federation`·`reproducibility`)으로 정렬하는 결정적 위치 설정 매트릭스. Phase 409(다국 BVLOS 비교) 자매 — `ComparisonAttribute` frozen dataclass(dimension enum·value/citation 비공백·evidence None|비공백 강제)·`SimulatorProfile`(frozen, key allowlist·전 축 정확 1회 커버 강제)·`comparison_matrix`(축 정렬 행, 각 행 `tools` 셀 `MappingProxyType` 읽기 전용)·`compare_dimension`(읽기 전용 매핑)·`EvidenceReport`(frozen, 카운트 교차검증). **정직성 결속**(CLAUDE.md): SDACS 능력 주장은 `evidence` 로 실재 리포 경로(`hybrid_collision_avoidance.py`·`drone_agent.py`·`swarm_autonomous_no_preplan.yaml`·`weather.py`·`federation_causal_delivery.py`·`monte_carlo.py`·`CITATION.cff`)를 가리켜 `test_sdacs_evidence_exists_on_disk` 가 디스크 실재 강제(허위 능력 주장 차단), 외부 도구는 본 리포에서 검증 불가하므로 `evidence` 부재로 비대칭을 정직 표면화(`test_only_sdacs_has_evidence`). 외부 능력은 공개 문헌 *설계 초점* 에 한정해 보수 요약(미검증 수치·우열 단정 금지). `submission_manifest()`(식별자 `SDACS-IBC-1`)가 Phase 465 공개 표준 스위트 `SDACS-SBS-10` 을 제출 단위로 묶되 **외부 도구 결과 수치는 담지 않고**(스위트 정의·기능적 위치 설정만 제출) 비교 *대상* 만 `comparators` 에 담아 주체(SDACS)는 `subject` 로 분리. SDACS 근거 커버리지 **7/8(88%)** — `domain` 축은 단일 모듈로 환원 불가해 근거 없음으로 정직 공시. 무작위성 0·부수효과 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--tool`·`--dimension`·`--evidence`·`--submission`). **code-reviewer 어드바이저 반영**: HIGH 2(① `submission_manifest` 의 `comparators` 가 주체 SDACS 를 포함해 subject 와 의미 모순 → `is_subject` 로 주체 제외 + 테스트를 `{bluesky, utrafman}` 검증으로 정정, ② `swarm_support` 의 citation/evidence 역할 혼동 + `traffic_model` 과 evidence 경로 중복 → citation 을 설계 문서로·evidence 를 군집 전용 YAML 로 교체해 축별 근거 구별)·MEDIUM 2(`comparison_matrix` 외곽 행 dict 가변 → 읽기 전용 주장 범위를 `tools` 매핑으로 정확화, `submission_manifest` 디스크 I/O 비명시 → docstring 명시)·LOW 1(`is_subject` 미테스트 → 테스트 추가) 반영. 단위 **52건 PASS**.
- **검증**: `tests/test_benchmark_comparison.py` **52건 PASS**(dataclass 불변식·전 축 커버·정직성 결속·디스크 실재 강제·외부 evidence 부재·읽기전용 매핑·EvidenceReport 카운트·제출 매니페스트 주체/대상 분리·SBS-10 결속·결정성·CLI 6종, 0.27s). 대상 기존 `.py` 무수정 순수 추가 → 회귀 무영향. 본 컨테이너 최소 의존성 설치 → 전체 수트는 CI 수집.
- **점검 발견(사용자 결정 필요)**: ① 작업 브랜치 `claude/fervent-babbage-b1as0u` 는 `origin/main` 대비 **336 ahead / 214 behind** — 다수 일일 점검 브랜치가 main 미머지 적체. ② 직전 머지 커밋 메시지가 `"1"` 로 비서술적(`e7f4bc6`·`1efadc8`) — 추적성 저하. 적체 PR 머지·정리는 사용자 승인 필요.
