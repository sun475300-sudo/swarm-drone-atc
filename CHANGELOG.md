...
### 추가 (feat/test) — 일일 점검 2026-06-19 (39차): ODYSSEY Track 🌏 Phase 407 ICAO UTM Framework Ed.4 적합성 자가 평가

- **작업 상황 점검**: 신규 컨테이너에서 의존성 설치 후 브랜치(`claude/fervent-babbage-z4l984`)가 `origin/main`(`6c525d2`)과 동기(클린 베이스) 확인. 전체 수트 **5,280 pass / 280 skip / 0 fail**(121s) 재현. 열린 PR 20건 점검 — 일일 점검 드래프트 4건(#368·#369 Phase 402 중복·#370 Phase 470·#371 Phase 403)과 dependabot 다수가 머지 대기 중. 중복 회피 위해 *어느 PR 도 다루지 않는* Track 🌏 International Standards(401-420)의 **Phase 407**(ICAO UTM Framework Ed.4 적합성 자가 평가)을 신규 착수.
- **Phase 407** (Track 🌏, 신규 코드) — `simulation/icao_utm_conformance.py` (신규) ICAO UTM Framework **Edition 4**(2023, *"A Common Framework with Core Principles for Global Harmonization"*)가 제시하는 전형적 UTM 시스템 핵심 역량을 SDACS 기능에 결정적으로 대응시키는 **적합성 자가 평가 매트릭스**. ICAO 가 제시한 **운영자 여정 10단계**(manage·register·plan·coordinate·authorise·monitor·inform·adapt·report·investigate, ICAO 공개 자료 확인)를 분류 축으로 SDACS 23개 UTM 역량을 평가. **Phase 401(EASA U-space 매핑)의 자매편** — 같은 기능 자산을 ICAO 의 축으로 재평가. 핵심: `status` 를 Phase 401 의 이진(충족/미충족)보다 정직한 **3값**(`conformant`·`partial`·`gap`)으로 두고, **정직성 결속** — `gap` ⟺ `sdacs_module is None`, 그 외(충족/부분)는 반드시 실재 모듈 인용을 `UTMCapability.__post_init__` 에서 강제(`test_cited_modules_exist_on_disk` 가 17개 인용 경로 디스크 실재 강제) — 모듈 없는 충족 주장 구조적 금지. `ConformanceReport`(가중 점수 conformant 1.0·partial 0.5·gap 0.0 → **83%**, 핵심 역량 **12/14**, `by_phase` MappingProxyType 읽기 전용). 조회 API `find_capability`·`capabilities_by_phase`·`capabilities_by_status`·`core_principles`·`gaps`·`conformance_matrix`(도구 간 교환 JSON). **갭 2종(전자등록 레지스트리 DB·사고조사 워크플로) 정직 표면화**. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--phase`·`--gaps`·`--core`).
- **code-reviewer 어드바이저 반영** (CRITICAL 0·HIGH 0·MEDIUM 3·LOW 2): MEDIUM ①`ConformanceReport.by_phase` 가 평범한 dict 로 직접 생성 시 frozen 임에도 내부 변형 가능 → `__post_init__` 에서 `MappingProxyType` 동결(`object.__setattr__`), ②단계별 `by_phase` 합이 집계 총계와 불일치해도 무검증 수용 → 비어있지 않으면 `(conformant,partial,gap)` 합 일치 교차검증 추가, ③`has_core_gap` 명칭이 갭만 가리키는 듯하나 실제 부분 충족도 포함 → `has_core_incomplete` 로 정정 + docstring 명세 + CLI 문구("핵심 미완전 있음"). LOW ①`capability_id` 내부 공백 미거부 → snake_case 강제(`" " in id` 거부), ②CLI 무인자 경로 미테스트 → `main([])` 스모크 추가. 추가로 임포트 시점 중복 식별자 차단 assert 게이트 추가.
- **검증**: `tests/test_icao_utm_conformance.py` **56건 PASS**(어드바이저 반영 전 51 → 5건 추가: 내부공백 id 거부·partial 핵심 트리거·by_phase 합 불일치 거부·직접생성 동결·CLI 무인자). 대상 기존 `.py` 무수정 순수 추가 → **전체 수트 5,280 pass / 280 skip / 0 fail 회귀 무영향** 재확인(본 컨테이너 실측, 121s). CLI `--report` 실측: 가중 83%·핵심 12/14·갭 2.
