...
[![Tests](https://img.shields.io/badge/Tests-396%20passed-brightgreen?style=for-the-badge)](tests/)
...
**EN:** 396 automated tests passed, 38,400+ Monte Carlo validations, 3 live demos (Python Dash + Standalone HTML + SC2), 99.9% collision reduction in all scenarios. A complete capstone project.

**KR:** 396개 테스트 통과, 38,400회 이상 몬테카를로 검증, 3개 라이브 데모로 완성된 캡스톤 프로젝트입니다.
</details>
...
┌──────────────────────────────────────────────────────────────┐
│  Layer 4 — 사용자 인터페이스                                   │
│  CLI (main.py)  ·  3D Dash 대시보드  ·  pytest 396개          │
└───────────────────────────┬──────────────────────────────────┘
                            │ 명령 / 결과
┌───────────────────────────▼──────────────────────────────────┐
...
> **11개 핵심 알고리즘**이 4개 계층에서 계층적으로 동작합니다. (Python 3,500+줄 + HTML/JS 2,897줄)
...
| 11 | 결과 저장소 | `simulation/result_store.py` | 80+ | JSON/CSV 내보내기 + 태그 기반 비교 |
...
│   ├── result_store.py              # JSON/CSV 결과 저장소 + 비교
...
| Phase 2: 구현 | 2026.03 | SimPy 시뮬레이터, pytest 396개, SC2 14,200회 검증 | ✅ 완료 |
