...
| **APF 충돌 회피 v3** | Spatial Hash O(N·k) + CPA 12초 예측 + 속도벡터 기반 회피(텔레포트 제거) + 지수 반발력 + 우선순위 양보 + 매 프레임 실행 |
...
| **ATC 관제 드론 v2** | 9대 관제 드론 — 실제 속도벡터 CPA 예측, 우선순위 기반 선제 고도분리, 긴급 감속 명령, 레이더·탐지선 시각화 |
