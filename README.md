...
> **🆕 최신 업데이트 (2026-06-05 · HYPER Phase 11+12+14+15+20 통합 · 107/108 E2E)** — **🤖 AI Copilot 자연어 명령**(규칙 기반 NLP 22 패턴: "7번 드론을 정지", "모든 드론 귀환", "비 켜", "시간 12시", 오프라인 동작 + 향후 Claude API hook), **🪟 Electron 분할 모드**(CmdOrCtrl+Shift+S: 군집+해양 좌우 동시, IPC 시간축 1초 주기 동기, ATC 명령 양쪽 브로드캐스트), **🖼 시나리오 갤러리 모달**(5 카테고리), **🌐 4언어 i18n 순환**(KO→EN→JA→ZH), **🚢 해양 ATC 관제 콘솔**(8 명령 + TTS), **🖥 데스크탑 v1.1 재빌드 + ASAR Phase 12 코드 검증**, 메인 3D **MEGA 9 Phase 통합**, **해양 PWA**, **CI 3-job**, **HYPER 플랜 Phase 10-50**, **`_sdacs` 99 API 문서화**. Playwright E2E **107/108 통과** (1-9: 61 + 해양 PWA: 6 + 해양 ATC: 7 + 갤러리+i18n: 11 + 데스크탑 Phase 12: 11 + Copilot: 11), 회귀 **4,140/4,140 통과**. 🌌 HYPER: [`docs/SIMULATOR_HYPER_PLAN.md`](docs/SIMULATOR_HYPER_PLAN.md) · API: [`docs/SDACS_API.md`](docs/SDACS_API.md) · 릴리스: [`docs/RELEASE_GUIDE.md`](docs/RELEASE_GUIDE.md) · 마스터: [`docs/SIMULATOR_MEGA_PLAN.md`](docs/SIMULATOR_MEGA_PLAN.md)
>
...
| --- | --- | --- | --- |
| 2026-06-05 13:18 | `9dae29b` | ci(sim-smoke): E2E job에 pytest-cov 설치 (addopts --cov/--no-cov 인식) | .github/workflows/sim-smoke.yml |
| 2026-06-05 13:14 | `f58f4f4` | ci(sim-smoke): E2E job에 numpy 설치 + xr_mission(Phase 17-19) 테스트 포함 | .github/workflows/sim-smoke.yml |
