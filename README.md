...
> **🆕 최신 업데이트 (2026-06-05 · HYPER Phase 11-28 전부 통합 · 157/158 E2E · 150+ API)** — **🚀 본 사이클 신규 5종**: ⚡ **Phase 13 WebGPU 50K Spatial Hash 스캐폴드**(256×256 격자, build/query ms 측정), 👥 **Phase 16 CRDT 다중 관제사**(Lamport 클럭 + last-writer-wins, peer 자동 추적, 향후 Yjs WebRTC 격상), 🛩 **Phase 22 Digital Twin Pixhawk**(MAVLink GLOBAL_POSITION_INT 28바이트 little-endian 파서, GPS→ENU 변환, 시뮬-실드론 매핑), 📋 **Phase 24 NOTAM hook**(임시 NFZ 자동 변환, JSON import), 🔋 **Phase 25 Battery Aging Model**(cycle count + voltage sag + cold derating). 기존 **🛡 Counter-UAS 4종**, **🌬 Wind Field Grid**, **🎭 적대 드론 4종**, **💃 Choreography 5종**, **🥽 WebXR VR**, **📼 Mission Recorder**, **🤖 AI Copilot 22 패턴**, **🪟 Electron 분할 모드**, **🖼 시나리오 갤러리**, **🌐 4언어 i18n**, **🚢 해양 ATC 콘솔**, **🖥 데스크탑 v1.1 빌드**. Playwright E2E **157/158 통과**, 회귀 **4,140/4,140 통과**. 🌌 HYPER: [`docs/SIMULATOR_HYPER_PLAN.md`](docs/SIMULATOR_HYPER_PLAN.md) · API: [`docs/SDACS_API.md`](docs/SDACS_API.md) · 릴리스: [`docs/RELEASE_GUIDE.md`](docs/RELEASE_GUIDE.md) · 마스터: [`docs/SIMULATOR_MEGA_PLAN.md`](docs/SIMULATOR_MEGA_PLAN.md)
>
...
| --- | --- | --- | --- |
| 2026-06-05 14:15 | `e9dbd0c` | fix(ci): E2E 잡에 pytest-cov 추가 — pyproject addopts --cov/--no-cov 인식 복구 | .github/workflows/sim-smoke.yml |
| 2026-06-05 14:13 | `92a806d` | fix(ci): simulate --output JSON + E2E numpy 설치 — main CI 2건 RED 복구 | .github/workflows/sim-smoke.yml, main.py |
