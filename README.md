...
>
> **26가지 시나리오** (강풍, 침입 드론, GPS 교란, 대규모 택배 배송 등)를 시뮬레이션해서 시스템이 실제 상황에서도 작동하는지 검증합니다.
>
> 👉 **[3D 시뮬레이터 바로 체험하기](https://sun475300-sudo.github.io/swarm-drone-atc/visualization/swarm_3d_simulator.html)** — 설치 없이 브라우저에서 즉시 실행!
...
| APF 충돌 회피 | 근접 드론 간 척력 기반 자동 회피 + NFZ 진입 감지 자동 회피 |
| NFZ / 회랑 / 패드 | 비행금지구역(적색 박스), 동서/남북 항로, 착륙패드 5개소 시각화 |
| 26개 시나리오 | 7개 카테고리 (기본/장애·위기/교통·공역/자연·환경/위협·보안/임무/극한) 최대 150대 |
| 속도 조절 | 0.25x ~ 5x 실시간 속도 배율 슬라이더 |
| EVENT LOG | 실시간 이벤트 로그 — 💥충돌/⚠️근접/↪️회피/📡어드바이저리/📵통신두절/🔴장애 |
| Fleet Tracker | 우측 패널 — 드론별 고도/속도/배터리 실시간 모니터링 |
| 바람/ROGUE | 기상교란 시 바람 효과, 침입드론 특수 시각효과 |
| 시작/정지/초기화 | 시뮬레이션 제어 버튼 + 경과 시간 표시 |
| 성능 최적화 | 100대 초과 시 자동 경량 렌더링 (glow 비활성, 저폴리곤, APF 빈도 조절) |

#### 시나리오 전체 목록 (26개)

| 카테고리 | 시나리오 | 드론 수 | 특징 |
|---------|---------|--------|------|
| **기본** | 기본 / 고밀도 / 대규모 이륙 / 초대형 군집 | 30~150 | 표준 운용 상황 |
| **장애/위기** | 비상 장애 / 배터리 위기 / 연쇄 장애 / 통신 두절 / 복합 장애 | 30~60 | 고장·배터리·통신 |
| **교통/공역** | 경로 충돌 / NFZ 포화 / 혼합 교통 / 회랑 혼잡 / 교차 교통 | 50~80 | 공역 과밀·경합 |
| **자연/환경** | 기상 교란 / 강풍 폭우 / 안개 저시정 / 열 상승기류 | 25~40 | 기상 악화 대응 |
| **위협/보안** | 침입 드론 / 군집 침입 / GPS 스푸핑 | 35~50 | 보안 위협 대응 |
| **임무** | 수색 구조 / 택배 러시 / 편대 비행 | 20~80 | 미션 특화 |
| **극한** | 극한 스트레스 / 최종 시험 | 120 | 바람+장애+침입 동시 |
...
| 날짜 | 시간 | 주요 변경 사항 | 커밋 |
|------|------|---------------|------|
| 2026-03-26 | 19:00 KST | 시나리오 26개 확장 (7카테고리), EVENT LOG, 경량 렌더링, 비전공자 친화 README | `a682f5b` |
