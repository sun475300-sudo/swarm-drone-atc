...
> **🆕 최신 업데이트 (2026-06-05 · HYPER Phase 11+12+14+15+17+19+20+21+28 통합 · 129/130 E2E)** — **🎭 적대 드론 정책 4종**(미끼·군집침투·통신방해·직접요격, 5계층 안전망 robustness 시연), **💃 Swarm Choreography 5종**(원형·V편대·격자·하트·SDACS 로고, 발표·홍보 영상용), **🥽 WebXR VR**(navigator.xr 감지 시 우측 하단 VR 버튼 자동 노출, Meta Quest/Vision Pro 지원), **📼 Mission Recorder 공유**(.sdacs-mission JSON export/import, 시나리오·드론·KPI·ATC 로그 포함), **🤖 AI Copilot**(22 자연어 패턴), **🪟 Electron 분할 모드**, **🖼 시나리오 갤러리 모달**, **🌐 4언어 i18n**, **🚢 해양 ATC 관제 콘솔**, **🖥 데스크탑 v1.1 빌드**, 메인 3D **MEGA 9 Phase**, **해양 PWA**, **CI 3-job**, **`_sdacs` 110+ API 문서화**. Playwright E2E **129/130 통과** (1-9: 61 + 해양 PWA: 6 + 해양 ATC: 7 + 갤러리+i18n: 11 + Phase 12: 11 + Copilot: 11 + WebXR+Mission: 9 + 적대+군무: 13), 회귀 **4,140/4,140 통과**. 🌌 HYPER: [`docs/SIMULATOR_HYPER_PLAN.md`](docs/SIMULATOR_HYPER_PLAN.md) · API: [`docs/SDACS_API.md`](docs/SDACS_API.md) · 릴리스: [`docs/RELEASE_GUIDE.md`](docs/RELEASE_GUIDE.md) · 마스터: [`docs/SIMULATOR_MEGA_PLAN.md`](docs/SIMULATOR_MEGA_PLAN.md)
>
