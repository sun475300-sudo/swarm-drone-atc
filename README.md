...
> **🆕 최신 업데이트 (2026-06-05 · HYPER Phase 11+12+14+15 통합 · 96/97 E2E · v1.1 재빌드)** — **🪟 Electron 분할 모드**(CmdOrCtrl+Shift+S: 군집+해양 좌우 동시, IPC 시간축 자동 동기 1초 주기, ATC 명령 양쪽 브로드캐스트), **🖼 시나리오 갤러리 모달**(5 카테고리: 추천·스트레스·기상·군용임무·도시UAM, ESC/G 단축키), **🌐 4언어 i18n 순환**(KO→EN→JA→ZH), **🚢 해양 ATC 관제 콘솔**(8 명령 + TTS), **🖥 데스크탑 v1.1 재빌드 + ASAR Phase 12 코드 검증 완료**(main.js 7 markers + preload.js 3 markers), 메인 3D **MEGA 9 Phase 통합**, **해양 PWA**, **CI 3-job**, **HYPER 플랜 Phase 10-50**, **`_sdacs` 96 API 문서화**. Playwright E2E **96/97 통과** (1-9: 61 + 해양 PWA: 6 + 해양 ATC: 7 + 갤러리+i18n: 11 + 데스크탑 Phase 12: 11), 회귀 **4,140/4,140 통과**. 🌌 HYPER: [`docs/SIMULATOR_HYPER_PLAN.md`](docs/SIMULATOR_HYPER_PLAN.md) · API: [`docs/SDACS_API.md`](docs/SDACS_API.md) · 릴리스: [`docs/RELEASE_GUIDE.md`](docs/RELEASE_GUIDE.md) · 마스터: [`docs/SIMULATOR_MEGA_PLAN.md`](docs/SIMULATOR_MEGA_PLAN.md)
>
