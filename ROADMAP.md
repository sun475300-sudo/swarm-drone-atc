...
- [x] **P734** — 리플레이·타임라인 스크러버 + 동기화 멀티뷰 고도화 — 기존 레코더/스크러버(`recorder`·`applyFrame`)에 `syncReplayViews()` 추가: 스크럽/재생 시 분석 평면도(Q2)·선택 드론 상세 패널이 리플레이 프레임에 **즉시 동기화**(타이머 대기 제거). `_sdacs.replayIndex` getter + 헤드리스 스모크 15/15 (2026-06-03)
