...
- [x] **P729** — 대규모 모드 로터·글로우 InstancedMesh 확장(B3) — `glowInst` 추가 draw call 1로 드론 수 무관, 헤드리스 스모크 검증 (2026-06-03)
- [~] **P730** — UI 국제화(B5, KO/EN 토글) 완료 — 모바일·터치 대응 미구현
- [ ] **P731** — 공역 레이어 패널 중복 통합(O1) + 두 시뮬레이터 공통 컴포넌트 추출(D1)
- [x] **P732** — 대규모 CPA 충돌예측 복원(B2) — `updateConflictViz` 공간 해시 그리드(O(N·k)) 재작성, 400대 컷오프 제거, `_sdacs.conflictPairs` 노출, 1k+ megaMode 충돌쌍 검출·시각화(상위 60쌍 렌더) (2026-06-04)
...
*Last updated: 2026-06-04 — Track D 웹 시뮬레이터: P732(대규모 CPA 충돌예측 복원 — 공간 해시 그리드) 완료, P729(B3 글로우 인스턴싱)·P730(B5 i18n, 모바일/터치 잔여) 상태 정정. Track C 배포 인프라: P714·P715·P718·P719 완료, P720(공개 베타) 진행 예정. P721 Electron 데스크탑 앱 + P722~P728 완료, P731·P733~P735 진행 예정. 테스트 3,830+개.*
