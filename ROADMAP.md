...
- [x] **P706** — 기여도 비교 실험 (vs ORCA, vs VO, vs 단일 CBS) (2026-05-31)
...
- [x] **P712** — 인증·권한(OAuth2, RBAC) 및 감사 로그 — `api/auth.py` JWT HS256 + Role 계층(ADMIN>OPERATOR>VIEWER) + `/auth/token` 엔드포인트 구현 (2026-05-31)
- [x] **P713** — 실시간 WebSocket 채널 — `simulation/ws_bridge.py` 2Hz 스트리밍 + FastAPI `/ws/telemetry` 완비 (2026-05-29)
- [x] **P714** — PostgreSQL + TimescaleDB 이력 저장, 30일 보존 — `deployment/db/001_init.sql` 스키마 (hypertable + retention policy) 완비 (2026-05-31)
- [x] **P715** — Docker Compose → Kubernetes Helm 차트 변환 — `deployment/helm/sdacs/` Chart.yaml·values.yaml·templates/ 완비 (2026-05-31)
- [x] **P716** — CI/CD 완비 — GitHub Actions 6개 워크플로우 (테스트 3-버전 매트릭스, lint, mypy, 재현성 검증, E2E smoke, Pages 배포) (2026-05-29)
- [x] **P717** — 부하 테스트 (100기 스웜 실시간 시각화) — `tests/load/locustfile.py` Locust 시나리오 + smoke runner 완비 (2026-05-31)
- [x] **P718** — 관측성 스택 (Prometheus + Grafana + Loki) — FastAPI `/metrics` 엔드포인트 + 5종 메트릭(requests, drones, conflicts, ws_subscribers, sim_duration) 완비 (2026-05-31)
- [x] **P719** — 보안 감사 (OWASP ZAP, 의존성 CVE 스캔) — `scripts/security_audit.sh` + `results/security_audit_latest.txt` 완비, 21 CVE 확인 (2026-05-31)
...
*Last updated: 2026-05-31 (P706·P712·P714·P715·P717·P718·P719 완료, Track B P706 비교실험 결과 `results/comparison/` 저장, Track C P712-P719 소프트웨어 구현 완료)*
