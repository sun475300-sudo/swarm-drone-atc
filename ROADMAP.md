...
소프트웨어 컴포넌트는 PR #43에서 구현 완료 (실기 비행 시험은 하드웨어 확보 후 진행).

- [x] **P691** — PX4 v1.15+ 펌웨어 파라미터 검증 모듈 (`simulation/px4_firmware_validator.py`) — PR #43
- [x] **P692** — Jetson Orin Nano MAVLink UART↔UDP 브릿지 (`simulation/onboard_bridge.py`) — PR #43
- [x] **P693** — ASTM F3411-22a Broadcast/Network Remote ID (`simulation/remote_id_broadcast.py`) — PR #43
- [x] **P694** — RTK-GPS RTCM3 파서 + AirspaceController 피드백 (`simulation/rtk_gps_handler.py`) — PR #43
- [x] **P695** — RTL/Geofence 5단계 Failsafe 체인 (`simulation/failsafe_manager.py`) — PR #43
- [x] **P696** — PTP/NTP <10ms jitter 스웜 시간 동기화 (`simulation/swarm_time_sync.py`) — PR #43
- [x] **P697** — Vicon/Optitrack VRPN→MAVLink HITL 브릿지 (`simulation/mocap_hitl_bridge.py`) — PR #43
- [ ] **P698** — 실외 소규모 스웜 비행 시험 (3-5기 정지비행·포메이션) ❌ 하드웨어 필요
- [ ] **P699** — 풍동·강우·저조도 환경 시나리오 실측 ❌ 하드웨어 필요
- [x] **P700** — FMEA 8항목 + HITL 통합 보고서 (`simulation/hitl_report_generator.py`) — PR #43

### Track B — 연구·논문화 (Phase 701-710)

목포대 캡스톤 결과물을 학술적 기여로 정제.

- [x] **P701** — AIAA SciTech 2027 투고 주제 확정 (CBS+APF 하이브리드 + Remote-ID 완전 준수)
- [x] **P702** — 선행 연구 서베이 30편 (MAPF/Reactive/UTM/Swarm 4개 버킷)
- [x] **P703** — 벤치마크 데이터셋 공개화 + JSON Schema (`benchmarks/_schema/`) — PR #38
- [x] **P704** — Reproducibility 패키지 (중앙화 RNG + lock file + 재현 스크립트) — PR #38
- [x] **P705** — 평가 메트릭 정형화 (NMR·PE·MSD·RID-CR·RTF) — PR #35/#36/#38
- [x] **P706** — 비교 실험 1,200 runs (ORCA/VO/CBS vs SDACS, RID-CR=1.0 확인) — PR #38/#42
- [x] **P707** — 논문 초안 §5 결과·§6 토론·§7 결론 채움 (`docs/paper/PAPER_DRAFT.md`) — PR #42
- [ ] **P708** — 내부 리뷰 3회 + 지도교수 피드백 반영 ⚠️ 사람 필요
- [ ] **P709** — 공식 투고 및 arXiv 프리프린트 업로드 ⚠️ 사람 필요
- [x] **P710** — IROS 2026 A0 컨퍼런스 포스터 (`docs/IROS2026_poster.html`) — PR #42

### Track C — 배포·서비스화 (Phase 711-720)

공역 관리자용 대시보드를 SaaS 수준으로 안정화.

- [x] **P711** — FastAPI + React CDN ATC 대시보드 (`docs/dashboard.html`) — PR #41
- [x] **P712** — JWT-HMAC RBAC + 감사 로그 (`api/auth.py`) — PR #37
- [x] **P713** — WebSocket 1kHz 서버 push (`api/fastapi_server.py`) — PR #37
- [x] **P714** — PostgreSQL + TimescaleDB 스키마 + 마이그레이션 (`scripts/db/`) — PR #40
- [x] **P715** — Docker Compose → Kubernetes Helm 차트 (`helm/sdacs/`) — PR #40
- [x] **P716** — CI/CD baseline-comparison job (GitHub Actions) — PR #35/#36/#38
- [x] **P717** — 부하 테스트 100 클라이언트 60 FPS (`scripts/load_test.py`) — PR #37
- [x] **P718** — Prometheus 메트릭 엔드포인트 (`GET /metrics`) — PR #35
- [x] **P719** — 보안 감사 스크립트 bandit + pip-audit + OWASP (`scripts/security_audit.py`) — PR #37
...
*Last updated: 2026-05-29 (Track A P691-697·P700 소프트웨어 ✅ | Track B P701-707·P710 ✅ | Track C P711-719 ✅ — 드래프트 PR #35~#43 머지 대기)*
