...
- [x] **P729** — 대규모 모드 로터·글로우 InstancedMesh 확장(B3) — `glowInst` 추가 글로우 인스턴스(draw call 1, 드론 수 무관) + 프러스텀 컬링·LOD (2026-06-03)
- [~] **P730** — UI 국제화(B5, KO/EN 토글) 완료 — 모바일·터치 대응 미구현(viewport 메타만 존재)
- [ ] **P731** — 공역 레이어 패널 중복 통합(O1) + 두 시뮬레이터 공통 컴포넌트 추출(D1)
- [x] **P732** — 대규모 CPA 충돌예측 복원(B2, 공간 해시) — `air.length>400` 하드 스킵 제거, EVADE_DIST 셀 격자 3×3 인접쌍 검사(O(N²)→O(N)) + 임박순 상위 60쌍 렌더 + `_sdacs.cpaPairs` 노출, 헤드리스 스모크 검증 (2026-06-03)
