...
- [~] **P711** — FastAPI 백엔드 완성 (`api/fastapi_server.py` 800+줄, 전체 엔드포인트 구현) — React 프론트엔드 미구현
- [x] **P712** — JWT/RBAC 인증·권한 및 감사 로그 — `api/auth.py` HS256 토큰·역할계층(admin/operator/viewer)·감사 링버퍼 구현, `/auth/token` `/admin/audit-log` 엔드포인트 추가 (2026-06-01)
...
- [x] **P717** — 부하 테스트 스크립트 — `scripts/load_test.py` asyncio+httpx 100 WebSocket 클라이언트 동시 연결, FPS/지연 측정 및 PASS/FAIL 판정 (2026-06-01)
- [x] **P718** — 관측성 스택 — `deployment/monitoring/docker-compose.monitoring.yml` Prometheus+Grafana+Loki+Promtail 완비, FastAPI `/metrics` 엔드포인트 추가 (2026-06-01)
- [x] **P719** — 보안 감사 스크립트 — `scripts/security_audit.py` bandit+pip-audit+safety CVE 스캔, JSON 리포트 생성 (2026-06-01)
...
*Last updated: 2026-06-01 (P706·P712·P717·P718·P719 완료 — Track C 배포 인프라 핵심 완성, P714·P715·P720 잔여)*
