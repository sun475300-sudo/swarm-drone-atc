...
- [x] **P729** — 대규모 모드 로터·글로우 InstancedMesh 확장(B3, 1k~10k) — glowInst 추가, draw call +1, 스모크 14/14 (2026-06-03)
- [x] **P730** — UI 국제화(B5, KO/EN 토글) — 해양 시뮬 data-i18n 35개 + applyLang() 토글, 스모크 18/18 (동적 텍스트는 잔여) (2026-06-03)
- [ ] **P731** — 공역 레이어 패널 중복 통합(O1) + 두 시뮬레이터 공통 컴포넌트 추출(D1)
- [x] **P732** — 대규모 CPA 충돌예측 복원(B2, 공간 해시) — `updateConflictViz`를 O(N²)→공간해시(3×3 셀)로 교체, `air.length>400` 조기종료 제거, megaMode MAX_PAIRS 120, `_sdacs.conflictPairs` API + 스모크 15/15 (2026-06-03)
