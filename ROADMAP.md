...
- [x] **P729** — 대규모 모드 로터·글로우 InstancedMesh 확장 — 글로우(기존)에 더해 로터 인스턴스 추가(단일 draw call, 드론 수 무관, 시간 기반 Y축 회전), 헤드리스 스모크 `rotorInstanceCount` 검증 통과 (2026-06-04)
- [~] **P730** — UI 국제화(B5, KO/EN 토글) + 모바일·터치 대응 — 메인 시뮬레이터 정적 UI(헤더·스탯·제어·카메라) KO/EN 토글 완료(해양 패턴 일관, `_sdacs.lang()`/헤드리스 스모크 검증), 터치는 OrbitControls 네이티브 + viewport meta. 전체 반응형 레이아웃·동적 텍스트 i18n 후속 (2026-06-04)
...
*Last updated: 2026-06-04 — Track D 웹 시뮬레이터: P729(대규모 모드 로터 InstancedMesh) 완료 + P730(메인 시뮬레이터 KO/EN i18n 토글, 해양 패턴 일관·헤드리스 스모크 검증) 정적 UI 완료. P731~P735 진행 예정. 이전: Track C P714·P715·P718·P719 완료, P720 진행 예정 / Track D P721~P728 완료. 테스트 3,830+개.*
