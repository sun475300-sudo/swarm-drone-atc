...
- [x] **P691** — PX4 v1.15+ 펌웨어 버전·파라미터 검증 SW (`px4_firmware_validator.py`) — Draft PR #43/45/51
- [x] **P692** — Jetson Orin Nano MAVLink UART↔UDP 브릿지 SW (`onboard_bridge.py`) — Draft PR #43/45/51
- [x] **P693** — ASTM F3411-22a Broadcast/Network Remote ID SW (`remote_id_broadcast.py`) — Draft PR #43/45/51
- [x] **P694** — RTK-GPS RTCM3 파서 + CRC-24Q + AirspaceController 피드백 SW (`rtk_gps_handler.py`) — Draft PR #43/45/51
- [x] **P695** — 5단계 Failsafe 체인 SW (RTL/Geofence) (`failsafe_manager.py`) — Draft PR #43/45/51
- [x] **P696** — NTP-스타일 <10ms jitter 스웜 시간 동기화 SW (`swarm_time_sync.py`) — Draft PR #43/45/51
- [x] **P697** — Vicon/Optitrack VRPN→MAVLink ATT_POS_MOCAP HITL 브릿지 SW (`mocap_hitl_bridge.py`) — Draft PR #43/45/51
- [ ] **P698** — 실외 소규모 스웜 비행 시험 (3-5기 정지비행·포메이션) ❌ 실기 하드웨어 필요
- [ ] **P699** — 풍동·강우·저조도 환경 시나리오 실측 ❌ 실기 하드웨어 필요
- [x] **P700** — FMEA 8항목 + HITL 통합 보고서 SW (`hitl_report_generator.py`) — Draft PR #43/45/51

### Track B — 연구·논문화 (Phase 701-710)

목포대 캡스톤 결과물을 학술적 기여로 정제.

- [x] **P701** — 논문 주제 확정 (`docs/paper/PAPER_TOPIC.md`) — Draft PR #35/38
- [x] **P702** — 선행 연구 서베이 30편 (`docs/paper/PRIOR_WORK_SURVEY.md`) — Draft PR #38
- [x] **P703** — 벤치마크 데이터셋 JSON Schema + 검증 스크립트 — Draft PR #38/48
- [x] **P704** — Reproducibility 패키지 + Docker + 시드 고정 (`docs/REPRODUCIBILITY.md`) — Draft PR #38
- [x] **P705** — 평가 메트릭 정형화 (`src/analytics/metrics.py`) — Draft PR #38
- [x] **P706** — 비교 실험 60 runs (ORCA/VO/CBS vs SDACS) — Draft PR #35/36/38/42
- [x] **P707** — 논문 초안 (`docs/paper/PAPER_DRAFT.md`) — Draft PR #38/42
- [ ] **P708** — 내부 리뷰 3회 + 지도교수 피드백 반영 ⚠️ 사람 필요
- [ ] **P709** — 공식 투고 및 arXiv 프리프린트 업로드 ⚠️ 사람 필요
- [x] **P710** — IROS 2026 컨퍼런스 포스터 (`docs/IROS2026_poster.html`) — Draft PR #42

### Track C — 배포·서비스화 (Phase 711-720)

공역 관리자용 대시보드를 SaaS 수준으로 안정화.

- [x] **P711** — FastAPI + React ATC 대시보드 (`docs/dashboard.html`) — Draft PR #41
- [x] **P712** — JWT HS256 + RBAC 인증 + 감사 로그 (`api/auth.py`) — Draft PR #37/40/45/47/50/51
- [x] **P713** — WebSocket 1kHz 서버 push — Draft PR #37
- [x] **P714** — TimescaleDB 스키마 + 30일 보존 정책 — Draft PR #40/47/50/51
- [x] **P715** — Kubernetes Helm 차트 (`helm/sdacs/`) — Draft PR #40/45/47/51
- [x] **P716** — Docker → ghcr.io CI/CD + Trivy CRITICAL 차단 — Draft PR #36/47/51
- [x] **P717** — Locust 부하 테스트 100기 60FPS — Draft PR #37/50/51
- [x] **P718** — Prometheus + Grafana + Loki 관측성 스택 — Draft PR #35/47/50/51
- [x] **P719** — pip-audit + Bandit + Gitleaks + Trivy IaC 보안 감사 — Draft PR #37/47/51
- [ ] **P720** — 공개 베타 오픈 (3개 파일럿 기관, 피드백 수집 4주) ❌ 파일럿 기관 필요
...
*Last updated: 2026-05-30 (Track A P691-P697·P700 SW 완료, Track B P701-P707·P710 완료, Track C P711-P719 완료 — 모두 Draft PR 대기 중. P698-P699 실기·P708-P709 사람·P720 파일럿 기관 필요)*
