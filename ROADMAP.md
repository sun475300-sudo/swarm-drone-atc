...
- [x] **P730** — UI 국제화(B5, KO/EN 토글) + 모바일·터치 대응 — `lang-btn` 토글 + `data-i18n` 속성 + `setLang()` 함수 + WS LIVE 상태 표시 완비 (2026-06-03)
- [ ] **P731** — 공역 레이어 패널 중복 통합(O1) + 두 시뮬레이터 공통 컴포넌트 추출(D1)
- [ ] **P732** — 대규모 CPA 충돌예측 복원(B2, 공간 해시) — 1k+ 환경 충돌쌍 시각화
- [x] **P733** — `ws_bridge` 실데이터 라이브 수신 토글(데모↔실측) — `applyLiveData()` + `_WS_PHASE_MAP` + WS 연결 시 드론 위치·배터리·페이즈 자동 동기화 + LIVE/DEMO 상태 표시 완비 (2026-06-03)
...
*Last updated: 2026-06-03 — Track D P730(KO/EN i18n 토글 + data-i18n + setLang) + P733(ws_bridge LIVE/DEMO 토글·applyLiveData 드론 위치 동기화) 완료. P729·P731·P732·P734 진행 예정. CI 3-버전 그린(4003 passed, 88% 커버리지). 테스트 4,003+개.*
