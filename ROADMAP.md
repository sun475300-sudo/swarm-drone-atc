...
- [x] **P733** — `ws_bridge` 실데이터 라이브 수신 토글(데모↔실측) — `swarm_3d_simulator.html` 데이터 소스 토글(데모↔실측 WS) + `applyLiveSnapshot()` ws_bridge 스냅샷 적용(좌표 pos[x,y,alt]→wx/wz/wy, phase·battery·stats) + 서버 미실행 시 graceful, 헤드리스 스모크 16/16 + 모의 WS 엔드투엔드 검증 (2026-06-03)
