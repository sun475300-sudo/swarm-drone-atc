...
- [x] **P729** — 대규모 모드 글로우 InstancedMesh 확장(B3) — `glowInst` 가산 블렌딩, draw call +1만 증가(드론 수 무관), 1k에서 DC≈689 유지 (2026-06-03)
- [~] **P730** — UI 국제화(B5, KO/EN 토글) — 정적 `data-i18n` 35개 + 동적 텍스트(로그·트랙행·상세패널·검증기록 `L()`/`vtLabel()`) 완료, 스모크 19/19 통과. 모바일·터치 대응 잔여 (2026-06-03)
