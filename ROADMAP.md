...
- [x] **P706** — 기여도 비교 실험 (vs ORCA, vs VO, vs 단일 CBS) — `scripts/comparison_experiment.py` 10 시나리오 × 4 방법 × 5 시드 = 200회 실험 + Mann-Whitney U 유의성 검정 + SDACS APF+CBS 하이브리드 어댑터로 NMR·MSD·AU 유의미 개선 확인 + 비교 리포트 생성 (2026-06-01)
...
- [x] **P712** — 인증·권한(OAuth2, RBAC) 및 감사 로그 — `api/auth.py` HS256 JWT + 3계층 RBAC(admin/operator/viewer) + 패스워드 그랜트·토큰 리프레시·감사로그 완전 구현, 29개 테스트 통과 (2026-06-01)
...
- [x] **P717** — 부하 테스트 완비 — `scripts/load_test.py` 20/50/100기 벤치마크 (20기 RTF=1.6x, 100기 RTF=0.3x, 메모리 8-12MB) (2026-05-31)
...
*Last updated: 2026-06-04 — 병렬 세션 머지 충돌 해소(P706·P712·P717 통합, auth.py OAuth2 채택). Track B: P706 비교 실험 완료. Track C 배포 인프라: P712(인증/RBAC 29테스트)·P714(TimescaleDB asyncpg·하이퍼테이블)·P715(Helm 8템플릿)·P717(부하 테스트)·P718(Prometheus·Grafana·Loki)·P719(bandit·pip-audit·trivy 보안 감사) 완료, P720(공개 베타) 진행 예정. Track D 웹 시뮬레이터: P721 Electron 데스크탑 앱 + P722~P730(해양 C1~C9·메인 B3/B4/B6/B9/B10·B5 i18n) 완료, P731~P735 진행 예정.*
