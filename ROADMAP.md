...
소프트웨어 컴포넌트 전체 구현 완료 (PR #43). 실기 하드웨어 연동은 현장 진행 필요.

- [x] **P691** — PX4 v1.15+ 펌웨어 검증 스크립트 (`simulation/px4_firmware_validator.py`)
- [x] **P692** — Jetson Orin Nano 컴패니언 MAVLink 브릿지 (`simulation/onboard_bridge.py`)
- [x] **P693** — ASTM F3411-22a Remote ID 방송/네트워크 (`simulation/remote_id_broadcast.py`)
- [x] **P694** — RTK-GPS 센티미터 측위 + AirspaceController 피드백 (`simulation/rtk_gps_handler.py`)
- [x] **P695** — Failsafe 로직 RTL/Geofence 5단계 체인 (`simulation/failsafe_manager.py`)
- [x] **P696** — PTP/NTP 스웜 시간 동기화 <10ms jitter (`simulation/swarm_time_sync.py`)
- [x] **P697** — Vicon/Optitrack HITL 브릿지 VRPN→MAVLink (`simulation/mocap_hitl_bridge.py`)
- [x] **P698** — 실외 3-5기 비행 시험 프레임워크 (`simulation/outdoor_flight_test.py`)
- [x] **P699** — 풍동·강우·저조도 환경 시나리오 + APF 자동 전환 (`simulation/environmental_scenario.py`)
...
- [x] **P701** — 논문 주제 확정 및 기여 포인트 3개 도출 (PR #35-#38)
- [x] **P702** — 선행 연구 서베이 (PR #38)
- [x] **P703** — 벤치마크 데이터셋 공개화 (PR #36)
- [x] **P704** — Reproducibility 패키지 (PR #36)
- [x] **P705** — 평가 메트릭 정형화 NMR/PE/RID-CR (PR #35)
- [x] **P706** — 비교 실험 60 runs × 4 methods (PR #38, #42)
- [x] **P707** — 논문 초안 §5 결과 채움 (PR #42)
- [ ] **P708** — 내부 리뷰 3회 + 지도교수 피드백 반영 ⚠️ 사람 필요
- [ ] **P709** — 공식 투고 및 arXiv 프리프린트 업로드 ⚠️ 사람 필요
- [x] **P710** — IROS 2026 컨퍼런스 포스터 (PR #42)

### Track C — 배포·서비스화 (Phase 711-720)

공역 관리자용 대시보드를 SaaS 수준으로 안정화.

- [x] **P711** — FastAPI + React ATC 대시보드 (PR #41)
- [x] **P712** — JWT-HMAC RBAC + 감사 로그 (PR #37)
- [x] **P713** — WebSocket 1kHz 서버 push (PR #37)
- [x] **P714** — PostgreSQL + TimescaleDB 스키마 + 마이그레이션 (PR #40)
- [x] **P715** — Docker Compose → Kubernetes Helm 차트 (PR #40)
- [x] **P716** — CI/CD baseline comparison job (PR #35, #36)
- [x] **P717** — 부하 테스트 100기 60FPS (PR #37)
- [x] **P718** — Prometheus 메트릭 엔드포인트 (PR #35)
- [x] **P719** — 보안 감사 스크립트 bandit + pip-audit (PR #37)
...
*Last updated: 2026-05-29 (Track A P691-P700 소프트웨어 구현 완료, Track B P701-P707/P710 완료, Track C P711-P719 완료)*
