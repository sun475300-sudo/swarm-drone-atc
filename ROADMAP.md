...
- [x] **P729** — 대규모 모드 로터·글로우 InstancedMesh 확장(B3, 1k~10k 추가 최적화) (2026-06-03)
- [~] **P730** — UI 국제화(B5, KO/EN 토글) — 해양 시뮬레이터 i18n 완료(2026-06-03), 메인 시뮬레이터 i18n·모바일·터치 대응 진행 예정
- [ ] **P731** — 공역 레이어 패널 중복 통합(O1) + 두 시뮬레이터 공통 컴포넌트 추출(D1)
- [x] **P732** — 대규모 CPA 충돌예측 복원(B2, 공간 해시) — `updateConflictViz` O(N²)·400대 컷오프 폐기 → 공간해시 O(N·k)로 1k+ 충돌쌍 복원, 순수 함수 `_conflictCandidates` 추출 + 헤드리스 결정론 검증(수렴/발산/단일) (2026-06-03)
