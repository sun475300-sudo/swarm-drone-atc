...
*Last updated: 2026-06-10 — **일일 점검 (독립 검증) + 16-PR 백로그 트리아지**: 신규 컨테이너에서 의존성 전체 설치 후 독립 회귀 = **3,970 pass / 254 skip / 0 fail (GREEN)** (초기 14 fail은 `dash` 미설치 단일 원인, 설치 후 전부 통과 — 실 회귀 0). main CI 5개 워크플로우(CI·Hash·Security·Pages·pages-build) 전부 success @655a135. 소스 내 실 TODO는 `raft/airspace_controller_ha.py` 2건뿐이며 이미 PR #227에서 처리 중. **백로그 트리아지**: obsolete 확인 — #116/#118/#120(`simulate --output`은 main.py:487/507에 이미 존재) + #87(P711 React는 `frontend/` 18파일로 이미 통합). 검토 대기 — #227(raft TODO 정리, clean) + #202/#203(dependabot vite 5→8·vitest 2→4 **메이저** 점프, 빌드 영향 검증 필요). 잔여 (사용자 환경 의존): Track A 실기 검증, P707 실측 실험 그래프, P709 IROS 2026 실제 투고, P755 창업. 코드 로드맵 **99.5%** · conflict 마커 0.*
