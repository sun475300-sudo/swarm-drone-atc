...
- [x] **Phase 404** 🌏 README·핵심 문서 EN 완역 — `README.en.md`(학술 인용 가능 수준, 수치 SSoT 정합) + 양 README 언어 토글 (2026-06-22, PR #425 일원화)
- [x] **Phase 405** 🌏 국제 벤치마크 비교 — `simulation/benchmark_comparison.py` SDACS↔BlueSky·U-TRAFMAN 10개 비교 축 정렬(정직성 결속·프로젝트 평가 공시), 60건 PASS (2026-06-22, PR #424 일원화; 중복 PR #428 superseded)
- [x] **Phase 411** 🌏 해외 파일럿 제안서 적합성 게이트 — `simulation/overseas_pilot_proposal.py` + `docs/standards/OVERSEAS_PILOT_PROPOSAL_GATE.md`. 밴드 411-420("해외 파일럿 제안서 3종 — 아세안 도서 배송·EU U-space 데모·미국 대학 연구 협력")의 칸. Phase 410(GUTMA 기여 가능성)과 달리 *특정 해외 실증 제안 한 건*이 상대 관할·기관에 제출될 형식·근거를 갖췄는지 결정적 게이트로 판정. 요건 6종(필수 4·권장 2)을 EASA 2019/947+U-space 2021/664·FAA Part 107+COA/BEYOND·CAAS Unmanned Aircraft Act 등 공개 규제에서 도출하고 명문 근거 결속. `assess` 우선순위 CRITICAL UNMET→NOT_READY > CRITICAL PARTIAL→NEEDS_WORK > 잔여 미완→NEEDS_WORK > 전부 MET→READY_TO_PROPOSE, `POLICY_MATRIX` 5칸을 테스트가 정확 일치 강제. **정직 공시**: 후보 3종(`shipped_proposals`)을 격상 없이 판정 → 세 후보 모두 현지 호스트/파트너(PP-03) 미확보가 공통 CRITICAL 결격이라 전부 `NOT_READY`(ASEAN 30%·US 50%·EU 55%). 기능 커버리지(PP-02)는 인용 모듈 디스크 실재로 결속(`missing_module_refs`). 자문, 부수효과 0·무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--criteria`·`--status`·`--policy`·`--manifest`). 단위 46건 PASS·code-reviewer 어드바이저 HIGH 2·MEDIUM 1·LOW 2 반영 (2026-06-22)
- [ ] **Phase 412-420** 🌏 Global Expansion 잔여 — 해외 파일럿 제안서 잔여(412-420)
...
- [~] **Phase 451-460** 🔬 Formal & Research Frontier — RL 일반화 연구·인증 가능 ML 조사 (451 ✅ EASA 신뢰 가능 AI 적합성 자가 평가 `simulation/easa_ai_conformance.py`, 33% 정직 공시; 452 ✅ RL 일반화 평가 프로토콜 적합성 게이트 `simulation/rl_generalization_protocol.py` + `docs/standards/RL_GENERALIZATION_PROTOCOL.md` — 451 이 핵심 갭으로 지목한 *일반화 검증* 축을 깊게: 신뢰할 수 있는 일반화 주장의 전제(학습/평가 서로소 분할·OOD 평가·다중 시드·결정적 베이스라인 대조·일반화 지표 정의 등) 11개 요건을 5단계로 명문화하고 리포 내 실재 증거 산출물 감사로 종합 판정. 임계 요건 1건이라도 absent 면 `NOT_ESTABLISHED`(점수가 판정 불앞당김)·정직성 결속 `absent` ⟺ `evidence is None`·인용 경로 디스크 실재 강제. 현 리포 `NOT_ESTABLISHED (36%)` 정직 공시(임계 갭: 서로소 분할·일반화 지표 정의)·유일 충족은 RL 자문 불변식(일반화 실패 무해화). 58건 PASS; 453 ✅ RL 자문 경계 정합성 게이트 `simulation/rl_advisory_boundary.py` — ML/RL 자산이 활성 제어·안전 결정 경로에서 격리됨을 라이브 임포트 감사(`audit_active_loop_imports`)로 집행, `BOUNDARY_ENFORCED (94%)` 임계 4/4 성립·위반 0, 69건 PASS (2026-06-22); 454 ✅ ML 애플리케이션 EASA Level 분류 게이트 `simulation/ml_application_classification.py`, 전 ML 자산 Level 1A 정직 공시, 54건 PASS (2026-06-24); 455 ✅ ML 데이터 관리(Data Management) 적합성 게이트 `simulation/ml_data_management.py` + `docs/standards/ML_DATA_MANAGEMENT.md` — 451 이 지목한 갭의 *입력측*(학습 데이터 라이프사이클)을 EASA Learning Assurance W-shape 좌측(요구·수집·전처리·검증) 12개 목표로 세분. 정직성 결속 `gap` ⟺ `evidence is None`·인용 산출물 디스크 실재 강제·부재 시 자동 강등. 현 리포 `DATA_NOT_READY (71%)` 정직 공시(기반 갭: 실세계 데이터 출처 — 시뮬 생성 한계). code-reviewer 어드바이저 반영. 77건 PASS (2026-06-24); 456-460 잔여)
- [x] **Phase 454** 🔬 ML 애플리케이션 EASA Level 분류 게이트 — `simulation/ml_application_classification.py` + `docs/standards/ML_APPLICATION_CLASSIFICATION.md`. Phase 451 이 최대 갭으로 지목한 `ml_application_classification`("EASA Level 1A/1B/2/3 분류 기록 없음, 인증 경로 진입 전제 미충족")을 정면으로 채우는 자매 모듈. SDACS ML 자산 4종(PPO 충돌 회피·하이브리드 APF+RL·도메인 무작위화·시뮬-실측 보정)을 `POLICY_MATRIX`(과업 배분 × 권한 보유 = 9칸 전함수 → Level 1A..3B)로 결정적 분류. 정직성 결속: `human_authority != none` ⟺ 비-AI `override_authority` 실재 경로 인용(권한자 없는 감독 주장 차단), 인용 ML·권한자 모듈 디스크 실재 강제. **핵심 발견(정직)**: 전 ML 자산 **Level 1A**(자문, 최저 인증 부담) — ML 은 항상 자문이고 안전-결정권은 결정적 5계층 안전망 보유. 낮은 Level 은 결함이 아니라 설계 선택의 보상. 자문·부수효과 0·무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--levels`·`--policy`·`--constituent`). code-reviewer 어드바이저 반영. 54건 PASS (2026-06-24). 병행 draft #435(452 일반화·453 자문 경계)와 서로 다른 갭이라 번호 454 비충돌
