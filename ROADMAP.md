...
- [x] **P706** — 기여도 비교 실험 — `scripts/comparison_experiment.py` ORCA/VO/CBS/SDACS 4방식 Mann-Whitney U 유의성 검증 + Markdown 보고서 자동 생성, `tests/test_comparison_experiment.py` 21개 통과 (2026-06-01)
...
- [x] **P712** — 인증·권한(OAuth2, RBAC) — `api/auth.py` HS256 JWT (stdlib-only) + 3단계 RBAC (admin/operator/viewer) + `/auth/token` · `/auth/me` 엔드포인트 구현, `tests/test_auth.py` 20개 통과 (2026-06-01)
...
- [x] **P717** — 부하 테스트 — `scripts/load_test.py` asyncio+httpx 기반 동시 사용자 시뮬레이션, p95 SLA 검증, 엔드포인트별 통계 리포트 (2026-06-01)
- [x] **P718** — 관측성 스택 (Prometheus) — `/metrics` 엔드포인트 Prometheus text 포맷 (http_requests, errors, ws_connections, runs 6개 메트릭), HTTP 미들웨어 카운터 자동 집계 (2026-06-01)
