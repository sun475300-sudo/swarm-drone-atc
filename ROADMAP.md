...
- [x] **P706** — 기여도 비교 실험 — 10개 시나리오 × 4 방법(ORCA/VO/CBS/SDACS) × 3 seed = 120 run, SDACS 37/210 유의 우위 (`results/comparison/COMPARISON_REPORT.md`) (2026-06-01)
...
- [~] **P711** — FastAPI 백엔드 완성 (`api/fastapi_server.py` 전체 엔드포인트 구현) — React 프론트엔드 미구현
- [x] **P712** — 인증·권한(OAuth2, RBAC) 및 감사 로그 — JWT HS256(`api/auth.py`), RBAC 4역할, 감사 로그(`api/audit_log.py`), 관리자 엔드포인트, 테스트 27개 (2026-06-01)
...
- [x] **P717** — 부하 테스트 — Locust locustfile(`tests/load/locustfile.py`) 3개 유저 클래스(REST/WS/ScenarioRun), 단위 테스트 7개 (2026-06-01)
- [x] **P718** — 관측성 스택 — Prometheus(`/metrics` 엔드포인트) + Grafana + Loki + Promtail docker-compose, 알림 규칙, 메트릭 테스트 12개 (2026-06-01)
...
*Last updated: 2026-06-01 (P706 비교 실험 완료, P712 JWT/RBAC/감사로그 구현, P717 Locust 부하테스트, P718 관측성 스택 완료)*
