...
- [x] **P732** — 대규모 CPA 충돌예측 복원(B2, 공간 해시) — `updateConflictViz()` 공간 해시 broad-phase (cellSize=EVADE_DIST). 가드 400→2000으로 완화. air>80에서 자동 해시 사용, ≤80은 단순 O(N²). `_sdacs.conflictPairs` API 노출. 1k 시나리오 헤드리스 검증 통과 (2026-06-03)
