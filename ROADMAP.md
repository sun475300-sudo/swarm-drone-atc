...
- [x] **Phase 211-220** — production 격상 (12 → 30 API) — `test_simulator_production_core_30.py` (핵심 30종 실 호출 set→get 왕복·구조화 반환 검증 + 30종 production 등급 게이트, E2E 2건) (2026-06-26)
- [x] **Phase 211-220 후속** — production 격상 완료 — beta 18종(NOTAM 4·WindField 6·Choreography 4·Forecast 4)을 E2E 검증 후 production 이동 (`tests/e2e/test_simulator_production_promotion.py` 10건 — 동작 9 + 분류 게이트 1). **production 94 → 112** (계획 12→30 의 +18 정합). API 게이트 재생성 GREEN (2026-07-02)
...
- [ ] **Phase 322·324-340** 🌍 Ecosystem & Open Source 잔여 — `@sdacs/core` npm·`sdacs-sim` PyPI 실 퍼블리싱·v2.0 API 안정화 *(어댑터 스펙 공개는 Phase 323 ✅ 안착)*
...
- [x] **Phase 404** 🌏 README·핵심 문서 EN 완역 — `README.en.md` 학술 인용 수준 영문 번역 (2026-06-25 main 안착)
- [x] **Phase 405** 🌏 국제 벤치마크 비교 (SDACS·BlueSky·U-TRAFMAN) — `simulation/benchmark_comparison.py` 결정적 비교 시나리오 적합성 게이트 (2026-06-25 main 안착)
...
- [x] **Phase 411** 🌏 해외 파일럿 제안서 적합성 게이트 — `simulation/overseas_pilot_proposal.py` + `docs/standards/OVERSEAS_PILOT_PROPOSAL_GATE.md` (2026-06-25 main 안착)
- [ ] **Phase 410·412-420** 🌏 Global Expansion 잔여 — GUTMA 기고·해외 파일럿 실제 제출·국제 벤치마크 실측 결과 제출 *(외부 기관 의존)*
...
- [x] **Phase 473** 🏛 국제 WG 의견서 포트폴리오 추적기 — `simulation/wg_opinion_portfolio.py` + `docs/standards/WG_OPINION_PORTFOLIO.md` (2026-06-25 main 안착)
...
- [x] **Phase 451-460** 🔬 Formal & Research Frontier — RL 일반화 연구·인증 가능 ML 조사 완료 (451 ✅ `easa_ai_conformance.py`; 452 ✅ `rl_generalization_protocol.py` RL 일반화 평가 프로토콜; 453 ✅ `rl_advisory_boundary.py` RL 자문 경계; 454 ✅ `ml_application_classification.py` EASA Level 분류; 455 ✅ `ml_data_management.py` ML 데이터 관리; 456 ✅ `explainability_conformance.py` 설명가능성; 457 ✅ `easa_operational_monitoring.py` 운영 모니터링·드리프트 대응 — 모두 2026-06-25 main 안착; 458 ✅ `ml_verification_validation.py` ML V&V; 459 ✅ `ml_safety_case.py` Safety Case; 460 ✅ `ml_lifecycle_governance.py` Lifecycle Governance — 2026-07-01 #479 안착, README 표와 정합)
...
- [x] **EUROCAE WG-105 군집 ConOps 의견서 (Standards 추가 의견서)** 🏛 — `simulation/eurocae_wg105_opinion.py` + `docs/standards/EUROCAE_WG105_SWARM_CONOPS_OPINION.md`. 실행 가능한 redline(Before→After 제안 변경)까지 완성. 동봉 문서의 절별 디스크 증거로부터 WG-01~WG-06 상태를 결정적 도출하고 판정은 Phase 472 `assess` 에 위임(DRY — 복제 0). redline 완성으로 WG-02 PARTIAL→MET 격상(초안 점수 0.8→0.95), WG-06 은 회람 기한 외부 의존으로 PARTIAL 상한 고정 → `NEEDS_WORK` 정직 공시(준비도 ≠ 제출, READY_TO_SUBMIT 은 외부 기한 확정 후에만). Phase 473 포트폴리오는 본 작성 결과를 위임 재사용(하드코딩 스냅샷 제거). 자문·부수효과 0(읽기 전용)·무작위성 0. code-reviewer 어드바이저 반영. 20건 PASS (2026-06-25)
- [x] **ISO/TC 20/SC 16 23629 시리즈 의견서 (Standards 잔여 완결)** 🏛 — `simulation/iso_tc20_sc16_opinion.py` + `docs/standards/ISO_TC20_SC16_23629_OPINION.md`. 국제 워킹그룹 의견서 잔여 마지막 1건을 실행 가능한 redline(Before→After 제안 변경)까지 완성 — ISO 23629-12:2022(UTM 서비스 공급자 요건)의 단일 항공기 암묵 전제에 대해 군집 *집합 운영 의도(aggregate operational intent)* 수리 + graceful degradation 요건을 제안. 동봉 문서의 절별 디스크 증거로부터 WG-01~WG-06 상태를 결정적 도출하고 판정은 Phase 472 `assess` 위임(DRY — 복제 0, Phase 474 자매 동형). WG-06 은 체계적 검토 기한 외부 의존으로 PARTIAL 상한 고정 → `NEEDS_WORK`(0.95) 정직 공시(준비도 ≠ 제출, READY_TO_SUBMIT 은 외부 기한 확정 후에만). Phase 473 포트폴리오는 본 작성 결과를 위임 재사용(하드코딩 스냅샷 제거) — 밴드 3건 전부 작성 완료·잔여 발목은 외부 기한(WG-06)뿐. 자문·부수효과 0(읽기 전용)·무작위성 0. code-reviewer 어드바이저 APPROVE(지적 0건 — 앵커 유일성·게이트 계약·pyflakes 검증). 22건 신규 PASS·연관 102건 PASS (2026-07-03)
- [x] **JARUS SORA 군집 보완 의견서 redline (밴드 첫 후보 완결 — 국제 의견서 3건 전부 작성 완료)** 🏛 — `simulation/jarus_sora_opinion.py` + `docs/standards/JARUS_SORA_SWARM_OPINION.md`. 밴드 471-480 첫 후보(Phase 472 `shipped_letter` 의 JARUS SORA 군집 보완 의견)를 산문 수준에서 실행 가능한 redline(Before→After 제안 변경)까지 완성 — SORA v2.0 의 단일 항공기 암묵 전제(Step #2 iGRC·Step #9 containment)에 대해 군집 *집합 운영 볼륨(aggregate operational volume)* 선언 + 잔여 편대 격납 유지(graceful degradation) 요건을 제안(ISO 23629 의견서의 집합 운영 의도와 용어 정합). 동봉 문서의 절별 디스크 증거로부터 WG-01~WG-06 상태를 결정적 도출하고 판정은 Phase 472 `assess` 위임(DRY — 복제 0, EUROCAE·ISO 자매 동형). Phase 472 `shipped_letter` 가 본 작성 결과를 위임 재사용(하드코딩 스냅샷 제거 — 지연 임포트로 순환 차단, 0.8 산문 초안 → 0.95 정직 격상), WG-06 은 WG 회람 기한 외부 의존 PARTIAL 상한 → `NEEDS_WORK`(0.95). 이로써 국제 의견서 3건 전부 redline 작성 완료, 포트폴리오 잔여 발목은 외부 기한(WG-06 ×3)뿐 — sandbox 보완 가능 항목 소진. 자문·부수효과 0(읽기 전용)·무작위성 0. code-reviewer 어드바이저 APPROVE(지적 0건 — 순환 임포트 양방향·앵커/센티넬 계약·구형 서술 잔존 검증). 23건 신규 PASS·연관 125건 PASS·인접 문서 회귀 98건 PASS (2026-07-03)
