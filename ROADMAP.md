...
| **ODYSSEY** (시뮬 Phase 401-500) | 46% | ██████░░░░░░ | 402-411·447-448·452-457·461-500 완료 — FAA/EASA/ICAO·RL/AI·연합운영·표준·Continuum (#449 안착) |
...
- [x] **Phase 323** 🌍 외부 시뮬레이터 어댑터 (BlueSky·U-TRAFMAN import/export) — `simulation/external_sim_adapter.py` + `docs/standards/EXTERNAL_SIM_ADAPTER.md`. 측지(WGS84)/SI 정규형(`AdapterScenario`/`Aircraft`/`Waypoint` frozen dataclass) ↔ **BlueSky `.scn`** 명령 스택(`CRE`/`ADDWPT`, 고도 ft·속도 kt 변환, SDACS 기준점·시나리오명은 `#` 메타 주석 보존) ↔ **U-TRAFMAN-호환 JSON**(운항·비행계획 개념 정렬, SI 유지로 무손실) 결정적 왕복 변환. WGS84 ENU 브리지(`geodetic_to_enu`/`enu_to_geodetic` Bowring 역해·`scenario_to_enu`). 정직성: 외부 시뮬 실구동 아닌 포맷 변환기 — 공개 `.scn` 문법 부분집합·U-TRAFMAN 개념 정렬 교환 포맷(Phase 405 기능비교와 별개 트랙). 경계 fail-fast 검증(위경도 범위·비유한·음수 속도·acid 중복·고아/인자부족 ADDWPT·U-TRAFMAN 필수키 누락). WGS84 ECEF→측지 Bowring 역해는 고극 위도 sin 분기로 수치 안정. 무작위성 0·부수효과 0. CLI(`--demo`·`--format`·`--self-test`). code-reviewer 어드바이저 HIGH 3(ADDWPT off-by-one·CRE acid 중복 손실·knot 상수 절단)·MEDIUM 3(Key→ValueError·고극 alt·self-test 전필드) 반영. 59건 PASS (2026-06-26)
- [ ] **Phase 324-340** 🌍 Ecosystem & Open Source 잔여 — `@sdacs/core` npm·`sdacs-sim` PyPI 실 퍼블리싱·v2.0 API 안정화
