...
- **Phase 323** ✅ 외부 시뮬레이터 어댑터 스펙 공개 — BlueSky·U-TRAFMAN 호환 import/export — `simulation/external_sim_adapter.py` + `docs/standards/EXTERNAL_SIM_ADAPTER_SPEC.md` (중립 교환 모델 IR·등거리 투영·결정적 물질화 `seed` 재현성·BlueSky `.scn` CRE/DEST 왕복·U-TRAFMAN 비행계획 JSON·`scenario_schema` 호환 역변환·CLI 3종, 30건 PASS) (2026-06-26)
