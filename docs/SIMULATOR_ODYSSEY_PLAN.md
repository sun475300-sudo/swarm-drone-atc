...
- **Phase 441** ✅ 5계층 안전망 TLA+ 명세 — 충돌 회피 우선순위 불변식 (2026-06-17, `specs/SafetyNetPriority.tla` + `simulation/safety_net_invariant.py` + `docs/SAFETY_NET_TLA_SPEC.md` — `failsafe_manager.py` 이산 Failsafe 사다리(NORMAL<WARN<LAND<RTL<DISARM<EMERGENCY)를 유한 상태 전이계로 형식화. 핵심 불변식 **SAFE(우선순위 비역전)**: 어떤 도달 가능 상태에서도 레벨 ≥ 활성 위험 요구 레벨(comm/battery/geofence/collision → `RequiredLevel`=worst-hazard-wins, `_transition` 래칫 대응). TLA+ 명세를 동형 인코딩한 Python 유한 모델 검사기 `check_invariant` 가 도달 가능 전 상태를 BFS 전수 탐색해 SAFE·MONOTONE·RestoreSafe 검증(표본 아닌 전수), 위반 시 최단 반례 경로 반환. collision 위험을 무시하도록 약화한 컨트롤러로 검사기 건전성 음성 테스트(반례 실제 검출). TLC 미실행이라 `.tla`↔Python 동치성은 수동 검토(Phase 442에서 기계 검증 예정)·이산 추상화 갭·단일 드론 범위를 정직 공시. `failsafe_manager.py` 무수정 순수 추가, 무작위성 0·결정적. code-reviewer 어드바이저 HIGH 2건 반영(양성 검사 비자명화 — 환경·컨트롤러 별개 전이로 과도 상태 포함 후 복원 가능성·비공허성 검증·DFS→BFS 최단 반례). 단위 20건)
...
- **Phase 444** ✅ CBS 완전성·최적성 조건 정리 (논문 §보강) (2026-06-16, `simulation/cbs_optimality.py` + `docs/CBS_COMPLETENESS_OPTIMALITY.md` — Conflict-Based Search(Sharon et al. 2015)의 최적성(O1 허용적 저수준·O2 최선 우선 고수준)·완전성(C1·C2 분기 건전·C3 무한 탐색) 조건을 형식화하고 `cbs_planner/cbs.py` 무수정으로 감사. 독립 BFS 기준해(`reference_optimal_steps`, A* 보다 높은 지평)로 맨해튼 휴리스틱 허용성·저수준 A* 최적성 교차 검증, `child_constraints`·`vertex_branching_is_sound` 로 정점 분기 건전성(두 자식 제약 논리합 = 충돌 술어의 정확한 부정)을 구조 검증. `CBSGuaranteeAudit` 가 본 구현의 완화점을 정직 공시 — R1 에지(swap) 충돌을 정점 제약으로 근사(B측 제약이 공허해져 완전성만 깨짐, 최적성 불변)·R2 노드/시간 상한으로 anytime bounded·R3 `id()` 비결정적 타이브레이크. 결론: 정점 충돌 인스턴스에서 종료 시 조건부 최적, 교과서적 완전성은 미보장. code-reviewer 어드바이저 HIGH 3건 반영(BFS 기준해 지평을 A*보다 높여 독립성 보강·`vertex_branching_is_sound` 를 실제 분기 산출물 구조 검증으로 강화·R1 공허 제약 메커니즘 명시). 무작위성 0·결정적. 단위 15건)
- **Phase 445** ✅ 불확실성 정량화 — Monte Carlo 신뢰구간 자동 리포트 (`simulation/uncertainty.py` — t-분포 `normal_ci`·percentile `bootstrap_ci`(`np.random.default_rng` 재현)·Wilson score `proportion_ci` 세 방법으로 MC 점추정에 신뢰구간 부여, 단위 테스트 통과)
- **Phase 446** ✅ 충돌 해결률 공식의 통계적 검정력 분석 (`simulation/power_analysis.py` — 단일 비율 z-검정·Cohen's h 효과크기 닫힌형식 근사로 "SDACS p1 > 베이스라인 p0" 주장의 검정력·최소 런 수 설계, 단위 테스트 통과)
