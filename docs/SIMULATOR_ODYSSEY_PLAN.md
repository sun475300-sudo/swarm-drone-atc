...
- **Phase 402** ✅ FAA UTM ConOps v2 정렬 — USS 역할 요건 갭 분석 (2026-06-18, `simulation/faa_utm_conops.py` — Phase 401(EASA U-space)의 자매 모듈. FAA UTM ConOps v2.0(2020-03)의 USS/FIMS/SDSP/OPERATOR 역할 요건 15종을 SDACS 기능에 결정적으로 대응. `FaaUtmCapability` frozen dataclass가 각 요건의 책임 역할·FAA 핵심(필수) 여부·ConOps 능력군·**실재하는 리포 모듈 경로**(`sdacs_module`)를 보유 — FAA 정부 게이트웨이(FIMS) 연동은 `None` 으로 **갭** 을 정직 표면화(`test_cited_modules_exist_on_disk` 가 14개 인용 경로 디스크 실재를 강제, `test_fims_data_exchange_is_a_known_gap` 가 갭을 명시). 전체 충족 14/15(93%)·핵심 8/9(89%, FIMS 갭으로 미완)임을 `conformance_report()` 가 결정적 집계. 조회 API `capabilities_by_role`·`required_capabilities`·`gaps`·`capability_matrix`(도구 간 교환용 JSON 행)·`find_capability`, `ConformanceReport`(frozen, `__post_init__` 카운트 불변식 검증·`by_role` MappingProxyType 읽기 전용). 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--conformance`·`--role`·`--gaps`·`--required`). code-reviewer 어드바이저 HIGH 2 검토 — by_role 직접 생성 시 가변 우회(`__post_init__` MappingProxyType 강제 래핑으로 수정 + 직접 생성 읽기 전용 회귀 추가)·capability_matrix 가변 dict 행(JSON 교환 목적상 의도 유지, 자매 401 동일). 단위 39건 PASS)
