...
- **Phase 462** ✅ ISO/TC 20/SC 16 (UAS) 표준 동향 추적 매트릭스 (2026-06-19, `simulation/iso_tc20_sc16_tracker.py` — ISO/TC 20/SC 16(*Unmanned aircraft systems*) 위원회가 발행·개발 중인 *외부* UAS 국제 표준의 현황을 결정적으로 추적하고 표준별 SDACS 정렬을 표면화. **자매 모듈과 경계(중복 없음)**: Phase 407(`icao_utm_conformance`)은 ICAO UTM Framework라는 단일 조화 문서에 대한 SDACS *역량* 자가 평가, Phase 470(`standardization_tracker`)은 SDACS가 *발신하는* 표준화 *기고*(우리 산출물) 추적인 반면, 본 모듈(462)은 *외부 ISO 표준 문서 자체*의 발행 동향(어떤 부가 언제 발행됐고 어떤 부가 개발 중인가)을 추적하고 표준별 SDACS 정렬을 본다. **근거(2026-06 ISO 카탈로그 스냅샷)**: ISO 21384 시리즈 — Part 1(일반 사양, 개발 중)·Part 2(제품 시스템, 개발 중)·Part 3:2023(운영 절차, 발행)·Part 4:2025(용어, 발행); ISO 23629(UTM) 시리즈 — Part 5:2023(기능 구조)·Part 7:2021(공간 데이터 모델)·Part 8:2023(원격 식별)·Part 9:2023(USP–사용자 인터페이스)·Part 12:2022(서비스 공급자 요건) 발행. `IsoStandard` frozen dataclass 9건. **정직성 결속**: `alignment` 3값(`aligned`·`partial`·`none`), `none` ⟺ `sdacs_module is None`, 그 외는 반드시 실재 모듈 인용(`test_cited_sdacs_modules_exist_on_disk`가 디스크 실재 강제) — 모듈 없는 정렬 주장을 구조적으로 금지. 발행 표준은 지정명에 발행 연도(:YYYY) 명시 강제. 정렬 점수 44%(정렬 2·부분 4·미정렬 3), 발행 78%(7/9). 조회 API `find_standard`·`standards_by_series`·`standards_by_stage`·`published_standards`·`gaps`·`tracking_matrix`(도구 간 교환 JSON), `TrackerReport`(`by_series` MappingProxyType 읽기 전용·단계/정렬 합 교차검증). 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--series`·`--published`·`--gaps`). code-reviewer 어드바이저 반영(HIGH 2·MEDIUM 1·LOW 2): HIGH ①`TrackerReport` 빈 `by_series`+`total>0` 무검증 통과 → `total>0` 조건으로 불완전 투영 거부, ②`standards_by_series` 미지 시리즈 조용한 빈 결과(`standards_by_stage` 와 비대칭) → `SERIES` 정본 집합 대조 후 ValueError(CLI `--series` 도 `choices=SERIES`); MEDIUM 발행 연도 정규식 `\b`→음수 lookahead `(?!\d)`; LOW WITHDRAWN 단계·`find_standard` 필드 라운드트립 테스트 추가. 단위 59건 PASS)
