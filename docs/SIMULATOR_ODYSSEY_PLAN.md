...
- **Phase 462** ✅ ISO/TC 20/SC 16 (UAS) 표준 동향 추적 매트릭스 (2026-06-19, `simulation/iso_tc20_sc16_tracker.py` + `docs/standards/ISO_TC20_SC16_TRACKING.md` — ISO 기술위원회 TC 20 산하 소위원회 SC 16(Unmanned aircraft systems)이 개발·발행한 국제 표준 8종(ISO 21384-1/-2/-3/-4·21895·23665·23629-5/-7)을 SDACS 기능에 결정적으로 대응시켜 정렬·갭을 추적하는 동향 매트릭스. Phase 401(EASA)·402(FAA)·407(ICAO)가 *운영/규제* 체계 정렬을 다룬다면 본 Phase 는 *국제 표준화(ISO)* 축을 별도 추적하는 자매편 — 같은 SDACS 자산을 ISO 표준의 렌즈로 재평가. `ISOStandard` frozen dataclass(`__post_init__` 가 standard_id↔series 온전한 접두 강제·범주/상태 allowlist·식별자 strip), `TrackingReport`(by_category MappingProxyType 직접 생성 시 동결·상태합·정렬합·범주합 교차 불변식). `status` 는 프로젝트 추적 스냅샷(`AS_OF=2026-06`)이며 권위는 ISO 카탈로그임을 명시(시기 주장 비의존, 번호·표제 안정성에만 의존). 정렬 4/8(50%), 정직한 갭 4종(21384-1/-2 하드웨어 사양·21384-4 용어집·23665 훈련) 표면화 — `test_cited_modules_exist_on_disk` 가 인용 4개 모듈 디스크 실재 강제, `gap ⟺ sdacs_module is None` 정직성 결속. 조회 API `find_standard`·`standards_by_category`·`standards_by_series`·`gaps`·`aligned_standards`·`published_standards`·`tracking_matrix`(도구 간 교환 JSON 행, MappingProxyType 읽기 전용). 적재 시점 중복 식별자 assert 방어. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--matrix`·`--report`·`--category`·`--gaps`·`--published`). code-reviewer 어드바이저 HIGH 3(by_category 직접 생성 시 MappingProxyType 미동결·series bare startswith 오인 위험→구분자 강제·적재 시점 중복 assert 누락)·MEDIUM 2(by_category 합 교차검증·rejection 테스트 비현실 입력)·LOW 2(padded/empty series 미검 분기·CLI 무테스트) 반영. 단위 44건 PASS)
