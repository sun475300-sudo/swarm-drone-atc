...
- **Phase 488** ✅ 보안 장기 지원 — CVE 대응 SLA + 핀 갱신 절차 (2026-06-19, `simulation/cve_response_policy.py` + `docs/standards/CVE_RESPONSE_SLA_POLICY.md` — Phase 481(`dependency_gate`, 기능 갱신 자동 머지 정책)의 자매편. CVE 한 건이 들어왔을 때 *대응 긴급도·SLA·핀 갱신 필요* 를 결정적 정책으로 명문화한다(같은 입력=같은 결정). CVSS v3.1 정성 등급 NVD 표준 절단점(`severity_from_cvss`, 0=NONE·<4 LOW·<7 MEDIUM·<9 HIGH·≥9 CRITICAL, 범위 밖 거부). `CveReport` frozen dataclass(`__post_init__` 비공백 id/package·CVSS 범위·노출 enum·`fix_available`↔`fixed_version` 대칭 정합 검증)·`assess` 결정 우선순위(archived→`OUT_OF_SCOPE`[SECURITY.md "Out of Scope"]·NONE→`MONITOR`·dev 강등 유효 NONE→`MONITOR`·유효 CRITICAL/HIGH→`PATCH_NOW`·그 외→`SCHEDULED`). **SECURITY.md 기준선 준수**: SLA 는 외부 접수 7일·해결 14일을 HIGH 기준선으로 삼아 심각도별 단조(CRITICAL 1/7·HIGH 3/14·MEDIUM 7/30·LOW 14/90), HIGH 해결 14일이 SECURITY.md 상한과 일치. **노출 차등**: dev 노출은 유효 심각도 1단계 강등(공급망 위험 완화). **정직성 결속**: `pin_refresh_required` 는 상류 수정 존재(`fix_available`) + 핀 대상 노출일 때만 참 — 수정 없으면 핀 불가, 긴급도만 유지. **자문이지 집행 아님**(부수효과 0). `POLICY_MATRIX` 10칸(5 심각도 × runtime·dev)을 테스트가 `assess` 와 정확 일치 강제. 무작위성 0·기존 모듈 무수정 순수 추가. CLI(`--policy`·`--demo`·`--manifest`). code-reviewer 어드바이저 HIGH 3(LOW·dev 강등 바닥이 모순된 `SCHEDULED`+(0,0) SLA·핀 갱신 True 방출 → eff NONE 가드로 `MONITOR` 일원화·매니페스트 baseline 이 HIGH ack 를 7d 로 오기 → 실제 SLA 표에서 파생)·MEDIUM 2·LOW 1 반영. 단위 45건 PASS)
