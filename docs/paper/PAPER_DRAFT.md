...
![Fig. 2](figs/fig2_nmr_heatmap.pdf)

**Fig. 2** — Near-Miss Rate (NMR × 10⁻⁴) across 7 standard scenarios and
4 methods, mean of 30 seeds per cell. SDACS hybrid (rightmost column)
is best or tied-best on **every scenario except sc 05 weather diversion**
(where VO's aggressive detour wins), and **has ≤ 1 near-miss per 10⁴
drone-pair-seconds** on 5/7 scenarios. ORCA/CBS are essentially tied
because both rely on the same underlying trajectory planner.

Generated by `docs/paper/figs/generate_figures.py` from
`results/full_sweep/summary.csv`.

### 5.4 Ablation (Fig. 3)

![Fig. 3](figs/fig3_per_method_bars.pdf)

**Fig. 3** — Per-method summary across the 4 headline metrics, each
normalized to the worst per column. SDACS hybrid pays a tiny PE cost
(bar 3) and a moderate MS cost (bar 4) in exchange for order-of-magnitude
improvements on NMR (bar 1) and MSD (bar 2).

**Ablation experiment (planned for camera-ready):** drop one SDACS
layer at a time:
- `no_global` — APF + Regulatory only
- `no_reactive` — CBS + Regulatory only
- `no_regulatory` — CBS + APF only

Hypothesis: each layer dominates on a distinct scenario subset. Adding
the per-scenario win count for each ablation is `[TODO]` for the final
version — the 4-way per-method view in Fig. 3 already suggests the
answer: the reactive layer earns the safety, the global layer earns
the efficiency, together they beat every single-layer baseline.

### 5.5 Scaling (Fig. 4)

![Fig. 4](figs/fig4_scaling.pdf)

**Fig. 4** — Real-Time Factor (log scale) vs. number of agents per
scenario. RTF > 1 means the simulator runs faster than real time.
All four systems stay well above the RTF = 1 line at the scenario
sizes tested (N ≤ 40 agents). Target claim is **RTF ≥ 5 at N = 100**
and RTF ≥ 1 at N = 500 — these need the stress scenarios (08-10, N =
40 → 200 → adversarial), which are not yet included pending runtime
optimization of the ORCA/VO reactive layers on large populations.

`[TODO]` Extend to N ∈ {50, 100, 200, 500} once stress scenarios are
in — see §8 open items #5.
