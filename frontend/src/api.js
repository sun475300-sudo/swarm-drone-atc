...
export async function runScenario(
  scenarioId,
  { seed = 42, durationS = null, method = "hybrid" } = {},
) {
  // 백엔드 RunScenarioBody: seed·method(orca|apf|cbs|hybrid)·duration_s(정수, 기본 60).
  // duration_s 는 정수만 허용 → 반올림. null 이면 생략하여 백엔드 기본값 사용.
  const body = { seed, method };
  if (durationS != null) body.duration_s = Math.round(durationS);
...
export function connectTelemetry({ onMessage, onStatus }) {
  // BASE 가 절대 origin 일 때만 사용, 아니면 현재 출처(브라우저) 사용.
  // node 테스트 환경에는 window 가 없으므로 가드한다.
  const origin =
    typeof window !== "undefined" ? window.location.origin : "ws://localhost:8000";
  const base = BASE && /^https?:\/\//.test(BASE) ? BASE : origin;
  // http://→ws://, https://→wss://
  const wsUrl =
