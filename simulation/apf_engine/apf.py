...
def repulsive_force_drone(
    ...
    # 속도 기반 보정: 접근 중이면 척력 증폭 (고밀도에서 더 적극적)
    relative_vel = own_vel - other_vel
    closing_speed = -np.dot(relative_vel, n)  # 양수 = 접근 중
    if closing_speed > 0:
        # 고밀도: 최대 5×, 일반: 최대 3×
        max_amp = 5.0 if d0 > 100 else 3.0
        amplification = min(1.0 + closing_speed / 3.0, max_amp)
        mag *= amplification
...
def compute_total_force(
    ...
    if params is None:
        ...
        if is_high_density:
            ...
            params = {k: APF_PARAMS[k] * (1 - t) + APF_PARAMS_WINDY[k] * t for k in APF_PARAMS}
        else:
            params = APF_PARAMS
    ...
    for neighbor in neighbors:
        F_total += repulsive_force_drone(
            own.position, neighbor.position, own.velocity, neighbor.velocity, params["k_rep_drone"], params["d0_drone"]
        )

    # 3. 장애물 척력
    for obs in obstacles:
        F_total += repulsive_force_obstacle(own.position, obs, params["k_rep_obs"], params["d0_obs"])
...
def batch_compute_forces(
    ...
    for own in states:
        ...
        neighbor_indices = [
            j for j in np.where((dists < comm_range) & (dists > 0))[0] if pool[j].drone_id != own.drone_id
        ]

        neighbors = [APFState(pool_positions[j], pool_velocities[j], pool[j].drone_id) for j in neighbor_indices]

        # 바람 속도 가져오기
        wind_speed = wind_speeds.get(own.drone_id, 0.0)

        forces[own.drone_id] = compute_total_force(own, goal, neighbors, obstacles, params, wind_speed)
