...
# `10.` 접두(DOI 네임스페이스, Handle 시스템에서 DOI 재단 전용)는 기관 핸들로
# 인정하지 않는다 — Zenodo DOI(`10.5281/zenodo.x`)가 기관 custodian 으로 오집계
# 되는 것을 막는다(정직성). 순수 숫자 핸들 접두(`20.500.12345/...`)는 보존.
_HANDLE = re.compile(r"^(?!10\.)[^\s/]+/[^\s/]+$")
