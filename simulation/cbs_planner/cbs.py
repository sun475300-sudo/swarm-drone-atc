...
class AStarNode:
    ...
    parent: AStarNode | None = field(compare=False, default=None)
...
def get_neighbors(node: GridNode, bounds: dict) -> list[GridNode]:
    ...
    for dx, dy, dz in moves:
        ...
        if (
            ...
            and bounds["y"][0] <= ny <= bounds["y"][1]
            and bounds["z"][0] <= nz <= bounds["z"][1]
        ):
...
def low_level_astar(
    ...
    while open_heap:
        ...
        for neighbor in get_neighbors(current.node, bounds):
            ...
            next_node = AStarNode(f=f_new, g=g_new, node=neighbor, t=next_t, parent=current)
            heapq.heappush(open_heap, next_node)

    return []  # 경로 없음


def detect_conflict(paths: dict[str, list[GridNode]]) -> Conflict | None:
...
def cbs_plan(
    ...
    Args:
        ...
        max_ct_nodes: CBS 트리 최대 노드 수
        max_astars: A* 호출 최대 횟수 (시간 제한)
    ...
    if astar_calls > max_astars:
        return initial_paths  # 시간 초과 시 현재 경로 반환

    root = CTNode(constraints=[], paths=initial_paths, cost=sum(len(p) for p in initial_paths.values()))
    ...
    while open_list and explored < max_ct_nodes:
        ...
        for constrained_id in [conflict.drone_a, conflict.drone_b]:
            ...
            new_path = low_level_astar(
                starts[constrained_id], goals[constrained_id], new_constraints, constrained_id, bounds
            )
            astar_calls += 1
            if new_path:
                ...
                child = CTNode(
                    constraints=new_constraints, paths=new_paths, cost=sum(len(p) for p in new_paths.values())
                )
