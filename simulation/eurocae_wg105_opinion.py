"""ODYSSEY Phase 474 — EUROCAE WG-105 군집 ConOps 의견서(작성 완료 초안) 검증.

Standards & Policy 트랙(Phase 461-480) 밴드 `471-480`("국내 표준(KS) 제안 1건
+ 국제 워킹그룹 의견서 3건")의 *국제 의견서 잔여 2건* 중 **첫 건**을 실행 가능한
redline 까지 완성한다. Phase 473(`wg_opinion_portfolio`)은 EUROCAE WG-105 후보를
초안(WG-02 PARTIAL·WG-06 UNMET → NEEDS_WORK, 점수 0.8)으로 *격상 없이* 드러냈다.
본 모듈은 동봉 의견서 문서(`docs/standards/EUROCAE_WG105_SWARM_CONOPS_OPINION.md`)를
...
- **본 모듈(474)** 은 *EUROCAE WG-105 의견서 그 자체* 를 작성·검증한다.
  각 요건 충족 여부를 동봉 문서의 *디스크 실재·필수 절* 로부터 결정적으로
  도출하고, 판정은 Phase 472 `assess` 를 *호출만* 한다(DRY — 복제 0).
...
- **외부 의존 천장(WG-06)**: 제출 채널은 문서화 가능하나 공식 회람 *기한* 은
  EUROCAE 일정에 의존한다. 따라서 채널 절이 실재해도 WG-06 은 **PARTIAL** 로
  상한이 고정된다(채널 ○·기한 ✗). 준비도(작성)와 제출 상태(외부 절차)는 독립.
...
CLI:
    python -m simulation.eurocae_wg105_opinion --evidence  # 기준별 증거·상태
    python -m simulation.eurocae_wg105_opinion --status    # 게이트 판정
    python -m simulation.eurocae_wg105_opinion --manifest  # JSON 매니페스트
"""
...
DOC_PATH: Path = (
    ...
    / "docs"
    / "standards"
    / "EUROCAE_WG105_SWARM_CONOPS_OPINION.md"
)

# 대상 WG·문서(사람이 읽는 라벨) — Phase 473 포트폴리오 라벨과 동일.
TARGET_LABEL = "EUROCAE WG-105 — 군집 ConOps 부속 의견"
...
def _note_for(status: str, anchor: str, sentinel: str | None, ceiling: str | None) -> str:
    ...
    if status == STATUS_PARTIAL and ceiling is not None:
        return "절 실재(채널 문서화). 회람 기한 외부 의존으로 PARTIAL 상한 고정"
    return f"절 '{anchor}' 실재 — 충족"


def authored_letter(doc_path: Path | None = None) -> OpinionLetter:
    """동봉 문서 증거로부터 EUROCAE WG-105 의견서를 구성한다(Phase 472 형식)."""
...
def manifest(doc_path: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema": "sdacs-eurocae-wg105-opinion",
        "version": "1.0",
        "phase": 474,
        "band": "471-480",
        "target": TARGET_LABEL,
...
def _cmd_evidence() -> None:
    print(f"EUROCAE WG-105 의견서 기준별 증거 (문서: {DOC_PATH.name})")
