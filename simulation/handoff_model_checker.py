...
def verify_handoff_safe(
    ...
    *BFS 도달 순서상 먼저 만난* 위반의 반례를 반환한다(불변식 우선이 아니라 깊이
    ...
    계약 주의: *같은 한 상태* 가 불변식 위반이면서 동시에 교착이면
    :func:`check_model` 은 불변식 검사를 먼저 평가하므로 ``reason="invariant"``
    를 반환하고 그 상태의 교착성은 보고하지 않는다. 따라서 본 함수의 ``True``
    결과는 "어떤 도달 상태도 불변식을 깨지 않고, *동시에* 어떤 도달 상태도 교착이
    아니다" 를 함께 보장하지만, ``False`` 일 때 두 속성을 독립적으로 인증하려면
    :func:`verify_handoff_deadlock_free` 를 별도로 호출하라.
    """
    return check_model(
