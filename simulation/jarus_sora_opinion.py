"""ODYSSEY 밴드 첫 후보 완결 — JARUS SORA 군집 보완 의견서 검증.

Standards & Policy 트랙(Phase 461-480) 밴드 `471-480`("국내 표준(KS) 제안 1건
+ 국제 워킹그룹 의견서 3건")의 *첫 후보(JARUS SORA)* 를 실행 가능한 redline
까지 완성한다. Phase 472(`intl_wg_opinion_gate`)의 `shipped_letter` 는 이
후보를 산문 수준(WG-02 PARTIAL·WG-06 UNMET → 0.8)으로 *격상 없이* 드러냈다.
본 모듈은 동봉 의견서 문서(`docs/standards/JARUS_SORA_SWARM_OPINION.md`)를
...
- **Phase 472 `intl_wg_opinion_gate`** 는 *의견서 한 건*이 채택될 형식·근거
  ...
  472 의 `shipped_letter` 는 본 모듈의 작성 결과를 위임 재사용한다(순환 없음
  — 지연 임포트).
- **Phase 473 `wg_opinion_portfolio`** 는 밴드 목표 3건을 *집계* 한다.
- **Phase 476 `SDACS_JARUS_WG105_OPINION.md`** 는 JARUS 대상 *행동 권고·일정*
  문서다 — redline 의견서가 아니다.
- **본 모듈** 은 *JARUS SORA 의견서 그 자체* 를 작성·검증한다. 각 요건 충족
  여부를 동봉 문서의 *디스크 실재·필수 절* 로부터 결정적으로 도출하고,
  판정은 Phase 472 `assess` 를 *호출만* 한다(DRY — 복제 0). 구조는 자매편
  `eurocae_wg105_opinion`(Phase 474)·`iso_tc20_sc16_opinion` 과 동형이다.
...
- **외부 의존 천장(WG-06)**: 회원기관 라우팅 채널(국토교통부 경유)은 문서화
  가능하나 차기 SORA 갱신 회람의 의견 접수 *기한* 은 JARUS WG 일정에 의존한다.
  따라서 채널 절이 실재해도 WG-06 은 **PARTIAL** 로 상한이 고정된다
  (채널 ○·기한 ✗). 준비도와 제출 상태는 독립.
- 따라서 현 상태 판정은 `NEEDS_WORK`(점수 0.95) — sandbox 도달 가능한 정직한
  천장이며 `READY_TO_SUBMIT` 은 외부 회람 기한 확정 후에만 가능하다.

무작위성 0 · 결정적 · 부수효과 0(읽기 전용) · 순수 추가(기존 모듈 수정은
Phase 472 `shipped_letter` 의 위임 전환 한 곳뿐).

CLI:
    python -m simulation.jarus_sora_opinion --evidence  # 기준별 증거·상태
    python -m simulation.jarus_sora_opinion --status    # 게이트 판정
    python -m simulation.jarus_sora_opinion --manifest  # JSON 매니페스트
"""
...
DOC_PATH: Path = (
    ...
    / "docs"
    / "standards"
    / "JARUS_SORA_SWARM_OPINION.md"
)

# 대상 WG·문서(사람이 읽는 라벨) — Phase 472 shipped_letter 라벨과 동일.
TARGET_LABEL = "JARUS SORA — 군집 운용(swarm) 보완 의견"
...
def _note_for(status: str, anchor: str, sentinel: str | None, ceiling: str | None) -> str:
    ...
    if status == STATUS_PARTIAL and ceiling is not None:
        return "절 실재(회원기관 채널 문서화). WG 회람 기한 외부 의존으로 PARTIAL 상한 고정"
...
def authored_letter(doc_path: Path | None = None) -> OpinionLetter:
    """동봉 문서 증거로부터 JARUS SORA 의견서를 구성한다(Phase 472 형식)."""
...
def manifest(doc_path: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema": "sdacs-jarus-sora-opinion",
        "version": "1.0",
        "band": "471-480",
        "target": TARGET_LABEL,
...
def _cmd_evidence() -> None:
    print(f"JARUS SORA 의견서 기준별 증거 (문서: {DOC_PATH.name})")
