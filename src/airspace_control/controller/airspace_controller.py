...
class AirspaceController:
    ...
    def __init__(
        ...
        self._lat_min_base = float(config.get("separation_standards", {}).get("lateral_min_m", 50.0))
        self._lat_min = self._lat_min_base
        self._vert_min = float(config.get("separation_standards", {}).get("vertical_min_m", 15.0))
        self._near_lat = float(config.get("separation_standards", {}).get("near_miss_lateral_m", 10.0))
        self._lookahead = float(config.get("separation_standards", {}).get("conflict_lookahead_s", 90.0))
        self._max_clear = int(config.get("controller", {}).get("max_concurrent_clearances", 500))
        self._wind_speed: float = 0.0  # 현재 풍속 (m/s), 외부에서 갱신

        # CBS planner 파라미터 (config 기반, 적응형 타임아웃 지원)
    ...
    def _update_dynamic_separation(self) -> None:
        ...
        if ws <= 5.0:
            ...
            wind_factor = 1.0 + 0.4 * (ws - 5.0) / 5.0
        elif ws <= 15.0:
            wind_factor = 1.4 + 0.2 * (ws - 10.0) / 5.0
        else:
            wind_factor = 1.6

        # 고밀도 기반 분리간격 (드론 수에 비례)
        active_count = len([d for d in self._active_drones.values() if d.is_active])
        if active_count > 150:
            density_factor = 1.5  # 150+ 드론 시 1.5배
        elif active_count > 100:
            density_factor = 1.3  # 100-150 드론 시 1.3배
        elif active_count > 50:
            density_factor = 1.1  # 50-100 드론 시 1.1배
        else:
            density_factor = 1.0

        # 최대값 사용
        self._lat_min = self._lat_min_base * max(wind_factor, density_factor)
    ...
    def _manage_holding_queue(self, t: float) -> None:
        ...
        self._holding_queue = [
            (et, did)
            for et, did in self._holding_queue
            if did in self._active_drones and self._active_drones[did].flight_phase == FlightPhase.HOLDING
        ]
    ...
    def _process_clearances(self, t: float) -> None:
        ...
        for req in batch:
            ...
            for nfz in self.planner.nfz_list:
                dist_to_nfz = float(np.linalg.norm(req.destination[:2] - nfz["center"][:2]))
            ...
            # 60초 윈도우 유지
            self._clearance_count_window = [ts for ts in self._clearance_count_window if t - ts <= 60.0]
            ...
            if self.analytics:
                ...
                self.analytics.record_event(event, t, drone_id=req.drone_id, voronoi_conflict=voronoi_conflict)

    def _cbs_plan_batch(self, batch: list) -> dict[str, list]:
        """CBS로 동시 요청 배치의 충돌 없는 경로 세트 계산"""
        bounds_m = max(
            abs(self.config.get("airspace", {}).get("bounds_km", {}).get("x", [-5, 5])[1]) * 1000,
            5000.0,
        )
        grid_bounds = {
    ...
    # SpatialHash는 균일 분포에서 O(Nk), KDTree는 O(N log N)
    # 1000대 이상에서 KDTree의 query_pairs가 더 효율적
            ...
            if cpa_dist < self._lat_min and cpa_t < self._lookahead:
                ...
                if target.goal is not None:
                    ...
                    try:
                        ...
                        if self.analytics:
                            self.analytics.record_event("REROUTE_FAILED", t, drone_id=target.drone_id, error=str(e))

    @staticmethod
    def _kdtree_query_pairs(positions: dict[str, np.ndarray], radius: float) -> list[tuple[str, str, float]]:
    ...
    def _pick_target(self, da: DroneState, db: DroneState) -> DroneState:
        ...
        pri_a = DRONE_PROFILES.get(da.profile_name, DRONE_PROFILES["COMMERCIAL_DELIVERY"]).priority
        pri_b = DRONE_PROFILES.get(db.profile_name, DRONE_PROFILES["COMMERCIAL_DELIVERY"]).priority
        if pri_a != pri_b:
    ...
    def _detect_lost_link(self, t: float) -> None:
        ...
        for did, drone in self._active_drones.items():
            ...
            if stale > TIMEOUT_S:
                ...
                if self.analytics:
                    self.analytics.record_event("LOST_LINK_DETECTED", t, drone_id=did, stale_s=stale)
    ...
    def remove_dynamic_nfz(self, nfz_id: str, t: float) -> bool:
        ...
        self.planner.nfz_list = [n for n in self.planner.nfz_list if n.get("id") != nfz_id]
