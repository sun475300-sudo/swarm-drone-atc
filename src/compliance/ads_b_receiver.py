...
class AdsbReceiver:
    ...
    def fetch_surrounding_traffic(self, center_lat: float, center_lon: float, radius_km: float) -> list:
        ...
        if random.random() < 0.01:
            intruder_lat = center_lat + (random.uniform(-1, 1) * radius_km * 0.009)
            intruder_lon = center_lon + (random.uniform(-1, 1) * radius_km * 0.009)
            
            traffic.append({
                "icao_address": f"A{random.randint(100000, 999999)}",
                "callsign": "MEDIVAC1",
                "lat": intruder_lat,
                "lon": intruder_lon,
                "alt": random.uniform(150, 300), # 유인기 고도
                "velocity": random.uniform(40, 60), # m/s
