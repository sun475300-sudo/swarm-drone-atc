"""ODYSSEY Phase 474 — EUROCAE WG-105 군집 ConOps 의견서 검증 테스트.

핵심 계약: 동봉 문서 디스크 증거로부터 기준별 상태 결정적 도출(절 실재→MET·
부재→UNMET·redline 센티넬 부재→PARTIAL·WG-06 외부 의존 PARTIAL 상한),
판정의 Phase 472 위임(복제 0), 현 산출물 정직 공시(redline 완성으로 0.95·
NEEDS_WORK — 외부 기한 천장), 결정론·매니페스트 JSON 직렬화·CLI.
"""
...
from simulation.eurocae_wg105_opinion import (
    DOC_PATH,
    TARGET_LABEL,
    assess_letter,
    authored_letter,
...
def test_wg06_is_partial_not_met_even_with_section():
    # 외부 회람 기한 의존 — 채널 절이 있어도 MET 으로 격상되지 않는다(정직 천장).
    by_id = {ev.criterion_id: ev.status for ev in evidence()}
...
def test_manifest_json_serialisable():
    ...
    assert restored["schema"] == "sdacs-eurocae-wg105-opinion"
    assert restored["phase"] == 474
