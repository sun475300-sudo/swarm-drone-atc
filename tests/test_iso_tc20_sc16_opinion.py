"""ODYSSEY Standards 잔여 완결 — ISO/TC 20/SC 16 23629 의견서 검증 테스트.
...
from simulation.iso_tc20_sc16_opinion import (
    DOC_PATH,
    TARGET_LABEL,
    assess_letter,
    authored_letter,
...
def test_wg06_is_partial_not_met_even_with_section():
    # 외부 체계적 검토 기한 의존 — 채널 절이 있어도 MET 으로 격상되지 않는다.
    by_id = {ev.criterion_id: ev.status for ev in evidence()}
...
def test_manifest_json_serialisable():
    ...
    assert restored["schema"] == "sdacs-iso-tc20-sc16-opinion"
...
def test_cli_manifest_emits_valid_json(capsys):
    ...
    assert parsed["schema"] == "sdacs-iso-tc20-sc16-opinion"
