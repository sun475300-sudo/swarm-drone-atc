...
def test_simulate_help_lists_output_flag():
    """simulate --help 에 --output 플래그가 노출되어야 한다.

    회귀 방어: 나이틀리 벤치마크 CI(ci.yml)가
    `simulate --output results/bench_*.json` 를 호출하므로
    이 플래그가 없으면 CI 가 RED 가 된다.
    """
...
def test_simulate_output_writes_json(tmp_path):
    """simulate --output 가 KPI 요약 JSON 파일을 생성해야 한다.

    나이틀리 벤치마크 CI 가 의존하는 산출물(results/bench_*.json)과
    동일한 경로를 재현한다. numpy 등 시뮬레이션 의존성이 없는 환경에서는 skip.
    """
    pytest.importorskip("numpy")
    out = tmp_path / "bench.json"
    result = _run_help(
        "simulate", "--duration", "2", "--drones", "3", "--output", str(out)
    )
    assert result.returncode == 0, f"stderr: {result.stderr}"
    assert out.exists(), "출력 JSON 파일이 생성되지 않음"

    import json

    payload = json.loads(out.read_text(encoding="utf-8"))
    # SimulationResult.to_dict() 핵심 KPI 키 + 실행시간
    assert "conflict_resolution_rate_pct" in payload
    assert "n_drones" in payload
